...
$script:QWEN_TTS_WEBUI_INSTALLER_VERSION = 175
...
Qwen TTS WebUI Installer 版本: `$script:QWEN_TTS_WEBUI_INSTALLER_VERSION
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        try {
            Invoke-WithLogging -LogPrefix `"qwen_tts_webui_launch_log`" -ScriptBlock {
            ...
            Write-Log `"Qwen TTS WebUI 出现异常, 已退出, 请检查控制台日志`" -Level ERROR
        }
        Read-Host | Out-Null
    }
}
