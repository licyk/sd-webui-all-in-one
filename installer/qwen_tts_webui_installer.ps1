...
$script:QWEN_TTS_WEBUI_INSTALLER_VERSION = 107
...
# 环境变量
$env:QWEN_TTS_WEBUI_PATH = "$script:InstallPath/$env:CORE_PREFIX"
$env:QWEN_TTS_WEBUI_ROOT = "$script:InstallPath/$env:CORE_PREFIX"
...
function Initialize-EnvPath {
    ...
    `$env:QWEN_TTS_WEBUI_PATH = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
    `$env:QWEN_TTS_WEBUI_ROOT = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
...
try {
    ...
    Write-Host `"launch_qwen_tts_webui_installer.ps1`" -ForegroundColor Yellow
...
function Add-Shortcut {
    ...
    if (!(Test-Path `"`$shortcut_icon`")) {
        Write-Log `"获取 Qwen TTS WebUI 图标中`"
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
        ...
        Invoke-WebRequest @web_request_params
        if (!(`$?)) {
            Write-Log `"获取 Qwen TTS WebUI 图标失败, 无法创建 Qwen TTS WebUI 快捷启动方式`"
            return
        }
    }

    Write-Log `"更新 Qwen TTS WebUI 快捷启动方式`"
...
function Test-WebUIEnv {
    ...
    if ((Test-Path `"`$PSScriptRoot/disable_check_env.txt`") -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 Qwen TTS WebUI 运行环境检测, 这可能会导致 Qwen TTS WebUI 运行环境中存在的问题无法被发现并解决`"
...
function Main {
    ...
    if (!(Test-Path `"`$PSScriptRoot/`$env:CORE_PREFIX`")) {
        Write-Log `"内核路径 `$PSScriptRoot\`$env:CORE_PREFIX 未找到, 请检查 Qwen TTS WebUI 是否已正确安装, 或者尝试运行 Qwen TTS WebUI Installer 进行修复`"
        Read-Host | Out-Null
        return
    }
    ...
    if (`$script:BuildMode) {
        ...
        if (`$req) {
            ...
            Write-Log `"Qwen TTS WebUI 出现异常, 已退出`"
        }
        Read-Host | Out-Null
    }
}
...
".Trim()
    ...
    Write-Log "$(if (Test-Path "$script:InstallPath/launch_qwen_tts_webui_installer.ps1") { "更新" } else { "生成" }) launch_qwen_tts_webui_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path "$script:InstallPath/launch_qwen_tts_webui_installer.ps1" -Value $content
}
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"Qwen TTS WebUI`"; p=`"`$env:CORE_PREFIX/main.py`"; t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
function Main {
    ...
    while (`$true) {
        ...
        switch (`$choice) {
            ...
            `"14`" { Start-Process `"https://github.com/licyk/sd-webui-all-in-one/blob/main/docs/qwen_tts_webui_installer.md`" }
            `"15`" { return }
        }
    }
}
