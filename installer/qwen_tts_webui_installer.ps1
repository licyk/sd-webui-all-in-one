...
"@)][string]$InstallPythonVersion,
    ...
    [Parameter(HelpMessage=@"
启用 Qwen TTS WebUI Installer 快照重建模式, 将根据快照文件重新准备 Python 版本并恢复环境
...
"@)][switch]$UseUpdateMode,
    ...
    [Parameter(HelpMessage=@"
禁用 Qwen TTS WebUI Installer 使用 PyPI 软件包镜像源, 使用 PyPI 官方源下载 Python 软件包
...
"@)][switch]$DisableUpdate,
    ...
    [Parameter(HelpMessage=@"
(仅在 Qwen TTS WebUI Installer 构建模式下生效, 并且只作用于 Qwen TTS WebUI Installer 管理脚本) 禁用 Hugging Face 镜像源, 不使用 Hugging Face 镜像源下载文件
"@)][switch]$DisableHuggingFaceMirror,

    [Parameter(HelpMessage=@"
(仅在 Qwen TTS WebUI Installer 构建模式下生效, 并且只作用于 Qwen TTS WebUI Installer 管理脚本) 使用自定义 Hugging Face 镜像源地址, 例如代理服务器地址为 https://hf-mirror.com, 则使用 -UseCustomHuggingFaceMirror "https://hf-mirror.com" 设置 Hugging Face 镜像源地址
...
"@)][string]$LaunchArg,
    ...
    [Parameter(HelpMessage=@"
(仅在 Qwen TTS WebUI Installer 构建模式下生效, 并且只作用于 Qwen TTS WebUI Installer 管理脚本) 禁用 Qwen TTS WebUI Hotpatcher 补丁系统
"@)][switch]$DisableHotpatcher,

    [Parameter(HelpMessage=@"
(仅在 Qwen TTS WebUI Installer 构建模式下生效, 并且只作用于 Qwen TTS WebUI Installer 管理脚本) 设置 Qwen TTS WebUI Hotpatcher runtime 通信端口, 有效范围为 1 ~ 65535
"@)][int]$HotpatcherPort,
...
"@)][switch]$EnableHotpatcherRuntime,
    ...
    [Parameter(HelpMessage=@"
(仅在 Qwen TTS WebUI Installer 构建模式下生效, 并且只作用于 Qwen TTS WebUI Installer 管理脚本) 创建 Qwen TTS WebUI 启动创建启动快捷方式
...
if (-not $script:InstallPath) {
    $script:InstallPath = Join-NormalizedPath $PSScriptRoot "qwen-tts-webui"
}

$script:InstallPath = Join-NormalizedPath $script:InstallPath
$script:HotpatcherPortSet = $PSBoundParameters.ContainsKey("HotpatcherPort")
...
$script:QWEN_TTS_WEBUI_INSTALLER_VERSION = 318
    ...
    foreach ($runtime_name in $runtime_names) {
        ...
        if (!(Test-Path -LiteralPath $source_path -PathType Container)) {
            if (($runtime_name -eq "python") -or ((Get-CurrentPlatform) -eq "windows")) {
                Write-Log "环境缺少 $display_name, 无法为 $LauncherName 准备 $display_name 环境, 请重新运行 Qwen TTS WebUI Installer 修复环境" -Level WARNING
                $runtime_ready = $false
            }
            continue
        }
...
function Invoke-Installation {
    ...
    if ($script:RestoreFromSnapshot) {
        ...
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "qwen-tts-webui", "install")
    ...
    if ($exit_code -ne 0) {
        ...
        Write-Log "运行 SD WebUI All In One ${operation_label} Qwen TTS WebUI 时发生了错误, 终止 Qwen TTS WebUI ${operation_label}进程, 可尝试重新运行 Qwen TTS WebUI Installer 重试失败的操作" -Level ERROR
        if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
        exit 1
    }
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 Qwen TTS WebUI Installer 的自动检查 Installer 更新功能`"
        return
    }
...
function Test-WebUIEnv {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 Qwen TTS WebUI 启动前环境检测, 这可能会导致 Qwen TTS WebUI 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
`"@)][string]`$LaunchArg,
    ...
    [Parameter(HelpMessage=@`"
禁用 Qwen TTS WebUI Hotpatcher 补丁系统
`"@)][switch]`$DisableHotpatcher,

    [Parameter(HelpMessage=@`"
设置 Qwen TTS WebUI Hotpatcher runtime 通信端口, 有效范围为 1 ~ 65535
`"@)][int]`$HotpatcherPort,
...
try {
    `$config = @{
        ...
        HotpatcherPortSet = `$PSBoundParameters.ContainsKey(`"HotpatcherPort`")
    ...
    (Import-Module (Join-Path `$PSScriptRoot `"modules.psm1`") -Function `"Join-NormalizedPath`", `"Get-TrimmedTextFile`", `"Resolve-CorePrefix`", `"Initialize-EnvPath`", `"Invoke-WindowsLongPathsStartupCheck`", `"Write-Log`", `"Format-CommandLineArgumentForLog`", `"Format-CoreCliCommandForLog`", `"Write-CoreCliFailureCommand`", `"Set-CorePrefix`", `"Get-Version`", `"Update-Installer`", `"Set-Proxy`", `"Set-PyPIMirror`", `"Set-HuggingFaceMirror`", `"Set-GithubMirror`", `"Set-uv`", `"Set-PyTorchCUDAMemoryAlloc`", `"Get-WebUILaunchArgs`", `"Add-Shortcut`", `"Test-MSVCPPRedistributable`", `"Test-WebUIEnv`", `"Set-Hotpatcher`", `"Update-SDWebUiAllInOne`", `"Get-CurrentPlatform`", `"New-AppShortcut`", `"Get-HelpMessage`", `"Test-PythonAndGit`", `"Get-NativeCommandExitCode`", `"Exit-ManagerScript`" -PassThru -Force -ErrorAction Stop).Invoke({
        ...
        `$script:HotpatcherPortSet = `$cfg.HotpatcherPortSet
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"qwen-tts-webui`", `"check-env`")
        & python -m sd_webui_all_in_one qwen-tts-webui check-env `$launch_args
        `$exit_code = Get-NativeCommandExitCode -Success `$?
        if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
    } else {
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"qwen-tts-webui`", `"launch`")
        & python -m sd_webui_all_in_one qwen-tts-webui launch `$launch_args
        `$exit_code = Get-NativeCommandExitCode -Success `$?
        if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
        if (`$exit_code -eq 0) {
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"qwen-tts-webui`", `"update`")
    Set-SnapshotCliArgs `$launch_args
    & python -m sd_webui_all_in_one qwen-tts-webui update `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
...
function Main {
    ...
    if (`$status) {
        ...
        try {
            ...
            Write-Log `"运行 Qwen TTS WebUI Installer 时出现了错误: `$_`" -Level ERROR
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"qwen-tts-webui`", `"reinstall-pytorch`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"qwen-tts-webui`", `"gui`", `"version-manager`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"qwen-tts-webui`", `"gui`", `"snapshot-manager`")
    & python -m sd_webui_all_in_one qwen-tts-webui gui snapshot-manager `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }

    Write-Log `"退出 Qwen TTS WebUI 快照管理脚本`"
...
function Update-Hotpatcher-Port {
    ...
    `$port_text = Read-SingleLineSettingEditor -Prompt `"Hotpatcher 运行时通信端口 (1-65535, 清空后保存可删除):`" -InitialText `$current_port
    ...
    if (([int]::TryParse(`$port_text, [ref]`$port)) -and (`$port -ge 1) -and (`$port -le 65535)) {
        ...
        Write-Log `"端口无效, 端口范围应为 1 ~ 65535`" -Level WARNING
    }
}
...
function Main {
    ...
    while (`$true) {
        Write-Log `"=== Qwen TTS WebUI 本地设置菜单 ===`"
        `$menu = @(
            ...
            @{ id=6;  n=`"应用启动参数`"; v=`$(Get-TextStatus `"launch_args.txt`") },
            @{ id=7;  n=`"Hotpatcher 补丁系统`"; v=`$(Get-ToggleStatus `"disable_hotpatcher.txt`" `"启用`" `"禁用`" `$true) },
            @{ id=8;  n=`"Hotpatcher 运行时服务`"; v=`$(Get-ToggleStatus `"enable_hotpatcher_runtime.txt`" `"启用`" `"禁用`") },
            @{ id=9;  n=`"Hotpatcher 运行时端口`"; v=`$(Get-TextStatus `"hotpatcher_port.txt`" `"自动`") },
            @{ id=10; n=`"创建启动快捷方式`"; v=`$(Get-ToggleStatus `"enable_shortcut.txt`" `"启用`" `"禁用`") },
            @{ id=11; n=`"PyPI 软件包镜像`"; v=`$(Get-ToggleStatus `"disable_pypi_mirror.txt`" `"启用`" `"禁用`" `$true) },
            @{ id=12; n=`"PyTorch CUDA 内存分配优化`"; v=`$(Get-ToggleStatus `"disable_set_pytorch_cuda_memory_alloc.txt`" `"启用`" `"禁用`" `$true) },
        ...
        switch (`$choice) {
            ...
            `"6`"  { Update-LaunchArgs }
            `"7`"  { Set-ToggleSetting `"disable_hotpatcher.txt`" `"Hotpatcher 补丁系统`" (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_hotpatcher.txt`")) }
            `"8`"  { Set-ToggleSetting `"enable_hotpatcher_runtime.txt`" `"Hotpatcher 运行时服务`" (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_hotpatcher_runtime.txt`"))) }
            `"9`"  { Update-Hotpatcher-Port }
            `"10`" { Set-ToggleSetting `"enable_shortcut.txt`" `"创建启动快捷方式`" (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_shortcut.txt`"))) }
            `"11`" { Set-ToggleSetting `"disable_pypi_mirror.txt`" `"PyPI 软件包镜像`" (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_pypi_mirror.txt`")) }
            `"12`" { Set-ToggleSetting `"disable_set_pytorch_cuda_memory_alloc.txt`" `"PyTorch CUDA 内存分配优化`" (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_set_pytorch_cuda_memory_alloc.txt`")) }
...
function global:Install-Hanafubuki {
    `$core_path = Join-NormalizedPath `$env:QWEN_TTS_WEBUI_INSTALLER_ROOT `$env:CORE_PREFIX
    if (!(Test-Path -LiteralPath `$core_path -PathType Container)) {
        Write-Log `"内核路径 `$core_path 未找到, 无法安装 Hanafubuki 启动器, 请检查 Qwen TTS WebUI 是否已正确安装, 或者尝试运行 Qwen TTS WebUI Installer 进行修复`" -Level ERROR
        return
    }

    `$installed = Install-HanafubukiPackage -InstallRoot `$env:QWEN_TTS_WEBUI_INSTALLER_ROOT -CachePath `$env:CACHE_HOME
    if (`$installed) {
        Move-PortableLauncherRuntime -InstallRoot `$env:QWEN_TTS_WEBUI_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"Hanafubuki 启动器`" | Out-Null
    }
}
...
这是 Qwen TTS WebUI 的安装后速查说明。完整安装、启动、更新和故障排查说明请阅读文档站。
Qwen TTS WebUI Installer 文档：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/

一、安装完成后先做什么
1. 运行 launch.ps1 启动 Qwen TTS WebUI。启动成功后，浏览器会打开或终端会显示访问地址。
2. 需要更新 Qwen TTS WebUI 时运行 update.ps1。
3. PyTorch 出问题或需要切换版本时运行 reinstall_pytorch.ps1。
4. 需要执行 Python、Pip、Git 命令时运行 terminal.ps1，或先运行 activate.ps1 激活环境。

二、常用脚本速查
- launch.ps1：启动 Qwen TTS WebUI。
- update.ps1：更新 Qwen TTS WebUI 和管理脚本。
...
- launch_qwen_tts_webui_installer.ps1：重新获取并运行最新 Qwen TTS WebUI Installer。
...
- git：安装器自带 Git。
- core：Qwen TTS WebUI 程序和主要数据目录。

四、遇到问题
- Windows 下运行 .ps1 后立刻闪退：先运行 configure_env.bat，再重新运行原脚本。
- 启动、安装或更新报错：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/troubleshooting/
- 不确定脚本参数：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/commands/
- 下载器或 Launcher 出错：https://licyk.github.io/sd-webui-all-in-one/tools/troubleshooting/

五、详细文档
- 启动与使用：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/usage/
- 配置与镜像：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/config/
- 维护与迁移：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/maintenance/
- 常用命令：https://licyk.github.io/sd-webui-all-in-one/installer/qwen-tts-webui/commands/
...
function Copy-InstallerConfig {
    ...
    if ((!($script:HotpatcherPortSet)) -and (Test-Path (Join-NormalizedPath $PSScriptRoot "hotpatcher_port.txt"))) {
...
function Use-BuildMode {
    ...
    if ($script:BuildWithLaunch) {
        ...
        if ($script:HotpatcherPortSet) { $launch_args.Add("-HotpatcherPort", $script:HotpatcherPort) }
