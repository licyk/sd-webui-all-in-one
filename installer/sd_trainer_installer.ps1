...
$script:SD_TRAINER_INSTALLER_VERSION = 537
$script:UPDATE_TIME_SPAN = 3600
# SD WebUI All In One 内核最低版本
$script:CORE_MINIMUM_VER = "2.4.10"
# PATH
& {
    ...
    foreach ($runtime_name in $runtime_names) {
        ...
        if (!(Test-Path -LiteralPath $source_path -PathType Container)) {
            if (($runtime_name -eq "python") -or ((Get-CurrentPlatform) -eq "windows")) {
                Write-Log "环境缺少 $display_name, 无法为 $LauncherName 准备 $display_name 环境, 请重新运行 SD Trainer Installer 修复环境" -Level WARNING
                $runtime_ready = $false
            }
            continue
        }
...
if exist .\hanamizuki.exe (
    ...
    Move-PortableLauncherRuntime -InstallRoot `$env:SD_TRAINER_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"绘世启动器`" | Out-Null
}


# 安装 Hanafubuki 启动器
function global:Install-Hanafubuki {
    `$core_path = Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX
    if (!(Test-Path -LiteralPath `$core_path -PathType Container)) {
        Write-Log `"内核路径 `$core_path 未找到, 无法安装 Hanafubuki 启动器, 请检查 SD Trainer 是否已正确安装, 或者尝试运行 SD Trainer Installer 进行修复`" -Level ERROR
        return
    }

    `$installed = Install-HanafubukiPackage -InstallRoot `$env:SD_TRAINER_INSTALLER_ROOT -CachePath `$env:CACHE_HOME
    if (`$installed) {
        Move-PortableLauncherRuntime -InstallRoot `$env:SD_TRAINER_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"Hanafubuki 启动器`" | Out-Null
    }
}
