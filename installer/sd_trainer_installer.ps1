...
function Add-Shortcut {
    ...
    if (!(Test-Path `"`$shortcut_icon`")) {
        ...
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
            OutFile = `"`$PSScriptRoot/sd_trainer_icon.ico`"
        }
        Invoke-WebRequest @web_request_params
        if (!(`$?)) {
...
function Download-Installer {
    ...
    ForEach (`$url in `$urls) {
        ...
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
            OutFile = `"`$PSScriptRoot/cache/sd_trainer_installer.ps1`"
        }
        Invoke-WebRequest @web_request_params
        if (`$?) {
...
".Trim()
    ...
    Write-Log "$(if (Test-Path "$script:InstallPath/launch_sd_trainer_installer.ps1") { "更新" } else { "生成" }) launch_sd_trainer_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path "$script:InstallPath/launch_sd_trainer_installer.ps1" -Value $content
}
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"SD Trainer`"; p=`"`$env:CORE_PREFIX/gui.py`"; t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
`$env:SD_TRAINER_PATH = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
...
function global:Install-Hanamizuki {
    ...
    if (Test-Path `"`$env:SD_TRAINER_INSTALLER_ROOT/`$env:CORE_PREFIX/hanamizuki.exe`") {
        Write-Log `"绘世启动器已安装, 路径: `$([System.IO.Path]::GetFullPath(`"`$env:SD_TRAINER_INSTALLER_ROOT/`$env:CORE_PREFIX/hanamizuki.exe`"))`"
        ...
        ForEach (`$url in `$urls) {
            ...
            try {
                ...
                Write-Log `"绘世启动器安装成功, 路径: `$([System.IO.Path]::GetFullPath(`"`$env:SD_TRAINER_INSTALLER_ROOT/`$env:CORE_PREFIX/hanamizuki.exe`"))`"
                Write-Log `"可以进入该路径启动绘世启动器, 也可运行 hanamizuki.bat 启动绘世启动器`"
                break
            }
            catch {
