...
$script:SD_TRAINER_INSTALLER_VERSION = 489
$script:UPDATE_TIME_SPAN = 3600
# SD WebUI All In One 内核最低版本
$script:CORE_MINIMUM_VER = "2.2.39"
# PATH
& {
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 SD Trainer Installer 的自动检查更新功能`"
        return
    }
...
function Test-WebUIEnv {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 SD Trainer 运行环境检测, 这可能会导致 SD Trainer 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
function Main {
    ...
    while (`$true) {
        ...
        `$menu = @(
            ...
            @{ id=14; n=`"补丁系统 Runtime`"; v=`$(Get-ToggleStatus `"enable_hotpatcher_runtime.txt`" `"启用`" `"禁用`") },
            @{ id=15; n=`"补丁系统端口`"; v=`$(Get-TextStatus `"hotpatcher_port.txt`" `"默认`") },
        ...
        switch (`$choice) {
            ...
            `"14`" { Set-ToggleSetting `"enable_hotpatcher_runtime.txt`" `"补丁系统 Runtime`" (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_hotpatcher_runtime.txt`"))) }
