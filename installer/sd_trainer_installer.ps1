...
"@)][string]$InstallPythonVersion,
    ...
    [Parameter(HelpMessage=@"
启用 SD Trainer Installer 快照重建模式, 将根据快照文件重新准备 Python 版本并恢复环境
...
"@)][switch]$UseUpdateMode,
    ...
    [Parameter(HelpMessage=@"
禁用 SD Trainer Installer 使用 PyPI 软件包镜像源, 使用 PyPI 官方源下载 Python 软件包
...
"@)][switch]$DisableUpdate,
    ...
    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 并且只作用于 SD Trainer Installer 管理脚本) 禁用 Hugging Face 镜像源, 不使用 Hugging Face 镜像源下载文件
"@)][switch]$DisableHuggingFaceMirror,

    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 并且只作用于 SD Trainer Installer 管理脚本) 使用自定义 Hugging Face 镜像源地址, 例如代理服务器地址为 https://hf-mirror.com, 则使用 -UseCustomHuggingFaceMirror "https://hf-mirror.com" 设置 Hugging Face 镜像源地址
"@)][string]$UseCustomHuggingFaceMirror,

    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 并且只作用于 SD Trainer Installer 管理脚本) 设置 SD Trainer 自定义启动参数, 如启用 --fast 和 --auto-launch, 则使用 -LaunchArg "--fast --auto-launch" 进行启用
"@)][string]$LaunchArg,

    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 并且只作用于 SD Trainer Installer 管理脚本) 创建 SD Trainer 启动创建启动快捷方式
...
"@)][switch]$DisableCUDAMalloc,
    ...
    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 且只作用于 SD Trainer Installer 管理脚本) 禁用 SD Trainer Installer 检查 SD Trainer 运行环境中存在的问题, 禁用后可能会导致 SD Trainer 环境中存在的问题无法被发现并修复
"@)][switch]$DisableEnvCheck,

    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 并且只作用于 SD Trainer Installer 管理脚本) 禁用 Hotpatcher 补丁系统注入
"@)][switch]$DisableHotpatcher,

    [Parameter(HelpMessage=@"
(仅在 SD Trainer Installer 构建模式下生效, 且只作用于 SD Trainer Installer 管理脚本) 设置 Hotpatcher runtime 通信端口, 可用范围为 1 ~ 65535
"@)][int]$HotpatcherPort,
...
if (-not $script:InstallPath) {
    $script:InstallPath = Join-NormalizedPath $PSScriptRoot "SD-Trainer"
}
...
& {
    $prefix_list = @("core", "SD-Trainer*", "lora-scripts*")
...
$script:SD_TRAINER_INSTALLER_VERSION = 524
$script:UPDATE_TIME_SPAN = 3600
# SD WebUI All In One 内核最低版本
$script:CORE_MINIMUM_VER = "2.3.2"
# PATH
& {
...
# 环境变量
$env:SD_TRAINER_PATH = Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX
$env:SD_TRAINER_ROOT = Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX
...
$env:SD_WEBUI_ALL_IN_ONE_LOGGER_NAME = "SD Trainer Installer"
...
function Write-Log {
    ...
    param(
        ...
        [string]$Level = "INFO",
        [string]$Name = "SD Trainer Installer"
    )
...
function Get-Version {
    $ver = $([string]$script:SD_TRAINER_INSTALLER_VERSION).ToCharArray()
    $major = ($ver[0..($ver.Length - 3)])
    $minor = $ver[-2]
    $micro = $ver[-1]
    Write-Log "SD Trainer Installer 版本: v${major}.${minor}.${micro}"
}
...
function Test-InstallerGitKernelSnapshot {
    ...
    if ($null -eq $Snapshot.kernel) {
        Stop-SnapshotRestore "快照缺少内核信息, 无法在 Installer 重建模式中恢复 SD Trainer"
    }
    if (-not [System.Convert]::ToBoolean($Snapshot.kernel.is_git_repo)) {
        Stop-SnapshotRestore "快照中的 SD Trainer 内核不是 Git 仓库, Installer 重建模式无法恢复该内核"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.url)) {
        Stop-SnapshotRestore "快照中的 SD Trainer 内核缺少 Git 远程地址, 无法从头重建环境"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.commit)) {
        Stop-SnapshotRestore "快照中的 SD Trainer 内核缺少 Git commit, 无法恢复到快照版本"
    }
}
...
function Resolve-SnapshotRebuildConfig {
    ...
    if ($snapshot_webui_type -ne "sd_trainer") {
        Stop-SnapshotRestore "快照 WebUI 类型不匹配: 期望 'sd_trainer', 实际 '$snapshot_webui_type'"
    }
...
function Invoke-Installation {
    ...
    if ($script:RestoreFromSnapshot) {
        ...
        $snapshot_path = Get-NormalizedFilePath $script:SnapshotPath
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "sd-trainer", "restore", $snapshot_path)
        & python -m sd_webui_all_in_one sd-trainer restore $snapshot_path $launch_params
        $operation_label = "恢复"
    } else {
    ...
    if ($exit_code -ne 0) {
        ...
        Write-Log "运行 SD WebUI All In One ${operation_label} SD Trainer 时发生了错误, 终止 SD Trainer ${operation_label}进程, 可尝试重新运行 SD Trainer Installer 重试失败的操作" -Level ERROR
        if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
        exit 1
    }
...
function Initialize-EnvPath {
    ...
    `$env:SD_TRAINER_PATH = Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX
    `$env:SD_TRAINER_ROOT = Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 SD Trainer Installer 的自动检查 Installer 更新功能`"
        return
    }

    if (`$script:BuildMode) {
        Write-Log `"SD Trainer Installer 构建模式已启用, 跳过 SD Trainer Installer 更新检查`"
        return
    }
    ...
    foreach (`$url in `$urls) {
        Write-Log `"检查 SD Trainer Installer 更新中`"
        try {
            `$web_request_params = @{
                Uri = `$url
                UseBasicParsing = `$true
                OutFile = (Join-NormalizedPath `$env:CACHE_HOME `"sd_trainer_installer.ps1`")
                TimeoutSec = 15
                ErrorAction = `"Stop`"
            }
            Invoke-WebRequest @web_request_params
            `$latest_version = [int]`$(
                Get-Content (Join-NormalizedPath `$env:CACHE_HOME `"sd_trainer_installer.ps1`") -Encoding UTF8 |
            ...
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试检查 SD Trainer Installer 更新中`" -Level WARNING
            } else {
                Write-Log `"检查 SD Trainer Installer 更新失败`" -Level ERROR
                return
            }
        }
    }

    if (`$latest_version -le `$script:SD_TRAINER_INSTALLER_VERSION) {
        Write-Log `"SD Trainer Installer 已是最新版本`"
        return
    }

    Write-Log `"调用 SD Trainer Installer 进行更新中`"
    & (Join-NormalizedPath `$env:CACHE_HOME `"sd_trainer_installer.ps1`") -InstallPath `$PSScriptRoot -UseUpdateMode
    ...
    Write-Log `"更新结束, 重新启动 SD Trainer Installer 管理脚本中, 使用的命令行参数: `$raw_params`"
...
function Test-PythonAndGit {
    ...
    if (`$python_cmd) {
        ...
        if (-not ((`$python_cmd) -and ((`$python_cmd.ToString().StartsWith(`$python_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$python_cmd.ToString().StartsWith(`$python_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
            Write-Log `"检测到当前使用的 Python 路径为 `${python_cmd}, 但未在 `${python_path_prefix} 或 `${python_extra_path_prefix} 这两个受 SD Trainer Installer 管理的 Python 路径, 即当前正在使用外部的 Python 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_sd_trainer_installer.ps1 修复运行环境`" -Level ERROR
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Python, 这将导致运行时发生异常, 请运行 launch_sd_trainer_installer.ps1 修复运行环境`" -Level ERROR
    }
    ...
    if (`$git_cmd) {
        if ((Get-CurrentPlatform) -eq `"windows`") {
            ...
            if (-not ((`$git_cmd) -and ((`$git_cmd.ToString().StartsWith(`$git_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$git_cmd.ToString().StartsWith(`$git_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
                Write-Log `"检测到当前使用的 Git 路径为 `${git_cmd}, 但未在 `${git_path_prefix} 或 `${git_extra_path_prefix} 这两个受 SD Trainer Installer 管理的 Git 路径, 即当前正在使用外部的 Git 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_sd_trainer_installer.ps1 修复运行环境`" -Level ERROR
            }
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Git, 这将导致运行时发生异常, 请运行 launch_sd_trainer_installer.ps1 修复运行环境`" -Level ERROR
    }
}
...
function Get-WebUILaunchArgs {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"launch_args.txt`")) -or (`$script:LaunchArg)) {
        if (`$script:LaunchArg) {
            ...
            `$launch_args = Get-TrimmedTextFile (Join-NormalizedPath `$PSScriptRoot `"launch_args.txt`") -Encoding UTF8
        }
...
function Add-Shortcut {
    ...
    `$IconConfig = @{
        `"windows`" = @{
            FileName = `"sd_trainer_icon.ico`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/sd_trainer_icon.ico`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/sd_trainer_icon.ico`")
        }
        `"linux`"   = @{
            FileName = `"sd_trainer_icon.png`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/sd_trainer_icon.png`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/sd_trainer_icon.png`")
        }
        `"macos`"   = @{
            FileName = `"sd_trainer_icon.icns`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/sd_trainer_icon.icns`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/sd_trainer_icon.icns`")
        }
    }
    if ((!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_shortcut.txt`"))) -and (!(`$script:EnableShortcut))) {
        return
    }
    Write-Log `"检测到 enable_shortcut.txt 配置文件 / -EnableShortcut 命令行参数, 开始检查 SD Trainer 快捷启动方式中`"
...
function Test-WebUIEnv {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 SD Trainer 启动前环境检测, 这可能会导致 SD Trainer 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
function Set-Hotpatcher {
    ...
    if (`$script:HotpatcherPortSpecified) {
        ...
        `$port_text = Get-TrimmedTextFile (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`") -Encoding UTF8
        `$port_value = 0
...
`"@)][switch]`$DisableUV,
    ...
    [Parameter(HelpMessage=@`"
设置 SD Trainer 自定义启动参数, 如启用 --fast 和 --auto-launch, 则使用 -LaunchArg ```"--fast --auto-launch```" 进行启用
`"@)][string]`$LaunchArg,

    [Parameter(HelpMessage=@`"
创建 SD Trainer 启动创建启动快捷方式
`"@)][switch]`$EnableShortcut,
...
`"@)][switch]`$DisableHotpatcher,
    ...
    [Parameter(HelpMessage=@`"
设置 Hotpatcher runtime 通信端口, 可用范围为 1 ~ 65535
`"@)][int]`$HotpatcherPort,
...
function Download-Installer {
    ...
    foreach (`$url in `$urls) {
        Write-Log `"正在下载最新的 SD Trainer Installer 脚本`"
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
            OutFile = (Join-NormalizedPath `$PSScriptRoot `"cache`" `"sd_trainer_installer.ps1`")
            TimeoutSec = 15
            ErrorAction = `"Stop`"
        }
        try {
            Invoke-WebRequest @web_request_params
            Write-Log `"下载 SD Trainer Installer 脚本成功`"
            break
        } catch {
            Write-Log `"下载 SD Trainer Installer 脚本失败: `$(`$_.Exception.Message)`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试下载 SD Trainer Installer 脚本`" -Level WARNING
            } else {
                Write-Log `"下载 SD Trainer Installer 脚本失败, 可尝试重新运行 SD Trainer Installer 下载脚本`" -Level ERROR
                return `$false
            }
        }
    }
...
function Main {
    ...
    if (`$status) {
        Write-Log `"运行 SD Trainer Installer 中`"
        `$arg = Get-LocalSetting
        `$extra_args = Get-ExtraArgs
        `$script_path = Join-NormalizedPath `$PSScriptRoot `"cache`" `"sd_trainer_installer.ps1`"
        try {
            Invoke-Expression `"& ```"`$script_path```" `$extra_args @arg`" -ErrorAction Stop
        }
        catch {
            Write-Log `"运行 SD Trainer Installer 时出现了错误: `$_`" -Level ERROR
...
".Trim()
    ...
    Write-Log "$(if (Test-Path (Join-NormalizedPath $script:InstallPath "launch_sd_trainer_installer.ps1")) { "更新" } else { "生成" }) launch_sd_trainer_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path (Join-NormalizedPath $script:InstallPath "launch_sd_trainer_installer.ps1") -Value $content
}
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"SD Trainer`"; p=(Join-NormalizedPath `$env:CORE_PREFIX `"gui.py`"); t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
function Main {
    ...
    while (`$true) {
        ...
        `$menu = @(
            ...
            @{ id=8;  n=`"创建启动快捷方式`"; v=`$(Get-ToggleStatus `"enable_shortcut.txt`" `"启用`" `"禁用`") },
            @{ id=9;  n=`"PyPI 软件包镜像`"; v=`$(Get-ToggleStatus `"disable_pypi_mirror.txt`" `"启用`" `"禁用`" `$true) },
            @{ id=10; n=`"PyTorch CUDA 内存分配优化`"; v=`$(Get-ToggleStatus `"disable_set_pytorch_cuda_memory_alloc.txt`" `"启用`" `"禁用`" `$true) },
            @{ id=11; n=`"启动前环境检测`"; v=`$(Get-ToggleStatus `"disable_check_env.txt`" `"启用`" `"禁用`" `$true) },
            @{ id=12; n=`"Installer 内核路径前缀`"; v=`$(Get-TextStatus `"core_prefix.txt`" `"自动`") },
        ...
        switch (`$choice) {
            ...
            `"7`"  { Update-LaunchArgs }
            `"8`"  { Set-ToggleSetting `"enable_shortcut.txt`" `"创建启动快捷方式`" (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_shortcut.txt`"))) }
            `"9`"  { Set-ToggleSetting `"disable_pypi_mirror.txt`" `"PyPI 软件包镜像`" (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_pypi_mirror.txt`")) }
            `"10`" { Set-ToggleSetting `"disable_set_pytorch_cuda_memory_alloc.txt`" `"PyTorch CUDA 内存分配优化`" (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_set_pytorch_cuda_memory_alloc.txt`")) }
            `"11`" { Set-ToggleSetting `"disable_check_env.txt`" `"启动前环境检测`" (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) }
            ...
            `"19`" { Start-Process `"https://licyk.github.io/sd-webui-all-in-one/installer/sd-trainer/`" }
...
function global:Install-Hanamizuki {
    ...
    if (!(Test-Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX))) {
        Write-Log `"内核路径 (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX) 未找到, 无法安装绘世启动器, 请检查 SD Trainer 是否已正确安装, 或者尝试运行 SD Trainer Installer 进行修复`" -Level ERROR
        return
    }
    ...
    if (Test-Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX `"hanamizuki.exe`")) {
        Write-Log `"绘世启动器已安装, 路径: `$([System.IO.Path]::GetFullPath((Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX `"hanamizuki.exe`")))`"
        ...
        foreach (`$url in `$urls) {
            ...
            try {
                ...
                Invoke-WebRequest @web_request_params
                Move-Item -Path (Join-NormalizedPath `$env:CACHE_HOME `"hanamizuki_tmp.exe`") (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX `"hanamizuki.exe`") -Force
                Write-Log `"绘世启动器安装成功, 路径: `$([System.IO.Path]::GetFullPath((Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX `"hanamizuki.exe`")))`"
                Write-Log `"可以进入该路径启动绘世启动器, 也可运行 hanamizuki.bat 启动绘世启动器`"
                break
            }
            catch {
...
if exist .\hanamizuki.exe (
    ...
    Set-Content -Encoding Default -Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `"hanamizuki.bat`") -Value `$content

    Write-Log `"检查绘世启动器运行环境`"
    if (!(Test-Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX `"python`" `"python.exe`"))) {
        if (Test-Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `"python`")) {
            Write-Log `"尝试将 Python 移动至 `$(Join-NormalizedPath env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX) 中`"
            Move-Item -Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `"python`") (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX) -Force
            if (`$?) {
            ...
            Write-Log `"环境缺少 Python, 无法为绘世启动器准备 Python 环境, 请重新运行 SD Trainer Installer 修复环境`" -Level ERROR
        }
    }

    if (!(Test-Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX `"git`" `"bin`" `"git.exe`"))) {
        if (Test-Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `"git`")) {
            Write-Log `"尝试将 Git 移动至 `$(Join-NormalizedPath env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX) 中`"
            Move-Item -Path (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `"git`") (Join-NormalizedPath `$env:SD_TRAINER_INSTALLER_ROOT `$env:CORE_PREFIX) -Force
            if (`$?) {
            ...
            Write-Log `"环境缺少 Git, 无法为绘世启动器准备 Git 环境, 请重新运行 SD Trainer Installer 修复环境`" -Level ERROR
        }
    }
...
function Initialize-HanamizukiEnv {
    ...
    if (!(Test-Path (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX "python" "python.exe"))) {
        if (Test-Path (Join-NormalizedPath $script:InstallPath "python")) {
            ...
            Write-Log "环境缺少 Python, 无法为绘世启动器准备 Python 环境, 请重新运行 SD Trainer Installer 修复环境"
        }
    }
    ...
    if (!(Test-Path (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX "git" "bin" "git.exe"))) {
        if (Test-Path (Join-NormalizedPath $script:InstallPath "git")) {
            ...
            Write-Log "环境缺少 Git, 无法为绘世启动器准备 Git 环境, 请重新运行 SD Trainer Installer 修复环境"
        }
    }
...
function Use-InstallMode {
    ...
    if ($script:BuildMode) {
        ...
        Write-Log "SD Trainer 环境构建完成, 路径: $script:InstallPath"
    } else {
        Install-Hanamizuki
        Initialize-HanamizukiEnv
        Write-Log "SD Trainer 安装结束, 安装路径为: $script:InstallPath"
    }

    Save-InstallResultSnapshot -CliName "sd-trainer" -WebUIPath (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX)
...
function Main {
    ...
    if ($script:UseUpdateMode) {
        ...
        if ($script:BuildMode) { Write-Log "SD Trainer Installer 构建模式已启用" }
        Write-Log "使用安装模式"
        Use-InstallMode
    }
}
