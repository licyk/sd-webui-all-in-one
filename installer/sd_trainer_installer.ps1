...
$script:SD_TRAINER_INSTALLER_VERSION = 394
...
1. 使用的安装器 (例如: SD Trainer Installer)
2. 问题的详细描述
3. 复现问题的具体步骤
4. 附上本日志文件
...
SD Trainer Installer 版本: `$script:SD_TRAINER_INSTALLER_VERSION
...
function Get-InstallerCmdletHelp {
    `$content = `"
使用:
    ./`$(`$script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-BuildMode] [-DisablePyPIMirror] [-DisableUpdate] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableHuggingFaceMirror] [-UseCustomHuggingFaceMirror <HuggingFace 镜像源地址>] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像源地址>] [-DisableUV] [-LaunchArg <SD Trainer 启动参数>] [-EnableShortcut] [-DisableCUDAMalloc] [-DisableEnvCheck]
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        try {
            Invoke-WithLogging -LogPrefix `"sd_trainer_launch_log`" -ScriptBlock {
            ...
            Write-Log `"SD Trainer 出现异常, 已退出, 请检查控制台日志`" -Level ERROR
        }
        Read-Host | Out-Null
    }
}
...
function Get-InstallerCmdletHelp {
    `$content = `"
使用:
    ./`$(`$script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-BuildMode] [-BuildWithBranch <SD Trainer 分支编号>] [-DisableUpdate] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像源地址>]
...
参数:
    ...
    -BuildWithBranch <SD Trainer 分支编号>
...
function Download-Installer {
    ...
    ForEach (`$url in `$urls) {
        ...
        if (`$?) {
            ...
            Write-Log `"下载 SD Trainer Installer 脚本失败`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
...
function Main {
    ...
    while (`$true) {
        ...
        switch (`$choice) {
            ...
            `"13`" { Start-Process `"https://github.com/licyk/sd-webui-all-in-one/blob/main/docs/sd_trainer_installer.md`" }
...
try {
    `$global:OriginalScriptPath = `$PSCommandPath
    `$global:LaunchCommandLine = `$MyInvocation.Line
    (Import-Module `"`$PSScriptRoot/modules.psm1`" -Function `"Join-NormalizedPath`", `"Initialize-EnvPath`", `"Write-Log`", `"Set-CorePrefix`", `"Get-Version`", `"Set-Proxy`", `"Get-CurrentPlatform`", `"Get-NormalizedFilePath`" -PassThru -Force -ErrorAction Stop).Invoke({
        `$script:OriginalScriptPath = `$global:OriginalScriptPath
        `$script:LaunchCommandLine = `$global:LaunchCommandLine
        Remove-Variable OriginalScriptPath -Scope Global -Force
        Remove-Variable LaunchCommandLine -Scope Global -Force
...
function Get-InstallerCmdletHelp {
    $content = "
使用:
    ./$($script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-InstallPath <安装 SD Trainer 的绝对路径>] [-PyTorchMirrorType <PyTorch 镜像源类型>] [-InstallPythonVersion <Python 版本>] [-UseUpdateMode] [-DisablePyPIMirror] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableUV] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像站地址>] [-InstallBranch <安装的 SD Trainer 分支>] [-BuildMode] [-BuildWithTorch <PyTorch 版本编号>] [-BuildWithTorchReinstall] [-BuildWithModel <模型编号列表>] [-BuildWithBranch <SD Trainer 分支编号>] [-BuildWithUpdate] [-BuildWithLaunch] [-NoPreDownloadModel] [-PyTorchPackage <PyTorch 软件包>] [-xFormersPackage <xFormers 软件包>] [-InstallHanamizuki] [-NoCleanCache] [-DisableUpdate] [-DisableHuggingFaceMirror] [-UseCustomHuggingFaceMirror <HuggingFace 镜像源地址>] [-LaunchArg <SD Trainer 启动参数>] [-EnableShortcut] [-DisableCUDAMalloc] [-DisableEnvCheck]
...
参数:
    ...
    -InstallPath <安装 SD Trainer 的绝对路径>
        ...
        例如: ./$($script:MyInvocation.MyCommand.Name) -InstallPath `"D:\Download`", 这将指定 SD Trainer Installer 安装 SD Trainer 到 D:\Download 这个路径
    ...
    -InstallBranch <安装的 SD Trainer 分支>
        ...
        例如: ./$($script:MyInvocation.MyCommand.Name) -InstallBranch `"kohya_gui_main`", 这将指定 SD Trainer Installer 安装 bmaltais/Kohya GUI 分支
