...
"@)][string]$InstallPythonVersion,
    ...
    [Parameter(HelpMessage=@"
启用 ComfyUI Installer 快照重建模式, 将根据快照文件重新准备 Python 版本并恢复环境
...
"@)][switch]$UseUpdateMode,
    ...
    [Parameter(HelpMessage=@"
禁用 ComfyUI Installer 使用 PyPI 软件包镜像源, 使用 PyPI 官方源下载 Python 软件包
...
"@)][switch]$DisableUpdate,
    ...
    [Parameter(HelpMessage=@"
(仅在 ComfyUI Installer 构建模式下生效, 并且只作用于 ComfyUI Installer 管理脚本) 禁用 Hugging Face 镜像源, 不使用 Hugging Face 镜像源下载文件
"@)][switch]$DisableHuggingFaceMirror,

    [Parameter(HelpMessage=@"
(仅在 ComfyUI Installer 构建模式下生效, 并且只作用于 ComfyUI Installer 管理脚本) 使用自定义 Hugging Face 镜像源地址, 例如代理服务器地址为 https://hf-mirror.com, 则使用 -UseCustomHuggingFaceMirror "https://hf-mirror.com" 设置 Hugging Face 镜像源地址
...
"@)][string]$LaunchArg,
    ...
    [Parameter(HelpMessage=@"
(仅在 ComfyUI Installer 构建模式下生效, 并且只作用于 ComfyUI Installer 管理脚本) 创建 ComfyUI 启动创建启动快捷方式
...
"@)][switch]$DisableEnvCheck,
    ...
    [Parameter(HelpMessage=@"
(仅在 ComfyUI Installer 构建模式下生效, 并且只作用于 ComfyUI Installer 管理脚本) 禁用 Hotpatcher 补丁系统
...
$script:COMFYUI_INSTALLER_VERSION = 513
    ...
    foreach ($runtime_name in $runtime_names) {
        ...
        if (!(Test-Path -LiteralPath $source_path -PathType Container)) {
            if (($runtime_name -eq "python") -or ((Get-CurrentPlatform) -eq "windows")) {
                Write-Log "环境缺少 $display_name, 无法为 $LauncherName 准备 $display_name 环境, 请重新运行 ComfyUI Installer 修复环境" -Level WARNING
                $runtime_ready = $false
            }
            continue
        }
...
function Test-InstallerGitKernelSnapshot {
    ...
    if ($null -eq $Snapshot.kernel) {
        Stop-SnapshotRestore "快照缺少内核信息, 无法在 Installer 重建模式中恢复 ComfyUI"
    }
    if (-not [System.Convert]::ToBoolean($Snapshot.kernel.is_git_repo)) {
        Stop-SnapshotRestore "快照中的 ComfyUI 内核不是 Git 仓库, Installer 重建模式无法恢复该内核"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.url)) {
        Stop-SnapshotRestore "快照中的 ComfyUI 内核缺少 Git 远程地址, 无法从头重建环境"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.commit)) {
        Stop-SnapshotRestore "快照中的 ComfyUI 内核缺少 Git commit, 无法恢复到快照版本"
    }
}
...
function Resolve-SnapshotRebuildConfig {
    ...
    if ($snapshot_webui_type -ne "comfyui") {
        Stop-SnapshotRestore "快照 WebUI 类型不匹配: 期望 'comfyui', 实际 '$snapshot_webui_type'"
    }
...
function Invoke-Installation {
    ...
    if ($script:RestoreFromSnapshot) {
        ...
        $snapshot_path = Get-NormalizedFilePath $script:SnapshotPath
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "comfyui", "restore", $snapshot_path)
        & python -m sd_webui_all_in_one comfyui restore $snapshot_path $launch_params
        $operation_label = "恢复"
    } else {
        $launch_params = Get-LaunchCoreArgs
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "comfyui", "install")
    ...
    if ($exit_code -ne 0) {
        ...
        Write-Log "运行 SD WebUI All In One ${operation_label} ComfyUI 时发生了错误, 终止 ComfyUI ${operation_label}进程, 可尝试重新运行 ComfyUI Installer 重试失败的操作" -Level ERROR
        if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
        exit 1
    }
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 ComfyUI Installer 的自动检查 Installer 更新功能`"
        return
    }
    ...
    foreach (`$url in `$urls) {
        ...
        try {
            ...
            if (`$i -lt `$urls.Length) {
                ...
                Write-Log `"检查 ComfyUI Installer 更新失败`" -Level ERROR
                return
            }
        }
    }
...
`"@
    `$docs_url = `"更多的帮助信息请阅读 ComfyUI Installer 使用文档: https://licyk.github.io/sd-webui-all-in-one/installer/comfyui/`"
...
function Test-PythonAndGit {
    ...
    if (`$python_cmd) {
        ...
        if (-not ((`$python_cmd) -and ((`$python_cmd.ToString().StartsWith(`$python_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$python_cmd.ToString().StartsWith(`$python_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
            Write-Log `"检测到当前使用的 Python 路径为 `${python_cmd}, 但未在 `${python_path_prefix} 或 `${python_extra_path_prefix} 这两个受 ComfyUI Installer 管理的 Python 路径, 即当前正在使用外部的 Python 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_comfyui_install.ps1 修复运行环境`" -Level ERROR
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Python, 这将导致运行时发生异常, 请运行 launch_comfyui_install.ps1 修复运行环境`" -Level ERROR
    }
    ...
    if (`$git_cmd) {
        if ((Get-CurrentPlatform) -eq `"windows`") {
            ...
            if (-not ((`$git_cmd) -and ((`$git_cmd.ToString().StartsWith(`$git_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$git_cmd.ToString().StartsWith(`$git_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
                Write-Log `"检测到当前使用的 Git 路径为 `${git_cmd}, 但未在 `${git_path_prefix} 或 `${git_extra_path_prefix} 这两个受 ComfyUI Installer 管理的 Git 路径, 即当前正在使用外部的 Git 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_comfyui_install.ps1 修复运行环境`" -Level ERROR
            }
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Git, 这将导致运行时发生异常, 请运行 launch_comfyui_install.ps1 修复运行环境`" -Level ERROR
    }
}
...
function Add-Shortcut {
    ...
    `$IconConfig = @{
        `"windows`" = @{
            FileName = `"comfyui_icon.ico`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/comfyui_icon.ico`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/comfyui_icon.ico`")
        }
        `"linux`"   = @{
            FileName = `"comfyui_icon.png`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/comfyui_icon.png`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/comfyui_icon.png`")
        }
        `"macos`"   = @{
            FileName = `"comfyui_icon.icns`"
            Urls     = @(`"https://modelscope.cn/models/licyks/sd-webui-all-in-one/resolve/master/icon/comfyui_icon.icns`", `"https://huggingface.co/licyk/sd-webui-all-in-one/resolve/main/icon/comfyui_icon.icns`")
        }
    }
...
function Test-WebUIEnv {
    ...
    if ((`$script:DisableEnvCheck) -or (Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`"))) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 ComfyUI 启动前环境检测, 这可能会导致 ComfyUI 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"check-env`")
        & python -m sd_webui_all_in_one comfyui check-env `$launch_args
        `$exit_code = Get-NativeCommandExitCode -Success `$?
        if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
    } else {
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"launch`")
        & python -m sd_webui_all_in_one comfyui launch `$launch_args
        `$exit_code = Get-NativeCommandExitCode -Success `$?
        if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
        if (`$exit_code -eq 0) {
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"update`")
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"custom-node`", `"update`")
    Set-SnapshotCliArgs `$launch_args
    & python -m sd_webui_all_in_one comfyui custom-node update `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
...
function Download-Installer {
    ...
    foreach (`$url in `$urls) {
        ...
        try {
            ...
            Write-Log `"下载 ComfyUI Installer 脚本失败: `$(`$_.Exception.Message)`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
...
".Trim()
    ...
    Write-Log "$(if (Test-Path (Join-NormalizedPath $script:InstallPath "launch_comfyui_installer.ps1")) { "更新" } else { "生成" }) launch_comfyui_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path (Join-NormalizedPath $script:InstallPath "launch_comfyui_installer.ps1") -Value $content
}
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"reinstall-pytorch`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"model`", `"install-library`")
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"gui`", `"version-manager`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"comfyui`", `"gui`", `"snapshot-manager`")
    & python -m sd_webui_all_in_one comfyui gui snapshot-manager `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }

    Write-Log `"退出 ComfyUI 快照管理脚本`"
...
function Update-Hotpatcher-Port {
    ...
    if (`$choice -eq `"1`") {
        ...
        `$port = Read-SingleLineSettingEditor -Prompt `"Hotpatcher 运行时通信端口 (1..65535):`" -InitialText `$current_port
        if (`$null -eq `$port) { Write-Log `"已取消修改 Hotpatcher 运行时通信端口`"; return }
        `$port_value = 0
...
function Main {
    ...
    while (`$true) {
        ...
        switch (`$choice) {
            ...
            `"19`" { Start-Process `"https://licyk.github.io/sd-webui-all-in-one/installer/comfyui/`" }
...
if exist .\hanamizuki.exe (
    ...
    Move-PortableLauncherRuntime -InstallRoot `$env:COMFYUI_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"绘世启动器`" | Out-Null
}


# 安装 Hanafubuki 启动器
function global:Install-Hanafubuki {
    `$core_path = Join-NormalizedPath `$env:COMFYUI_INSTALLER_ROOT `$env:CORE_PREFIX
    if (!(Test-Path -LiteralPath `$core_path -PathType Container)) {
        Write-Log `"内核路径 `$core_path 未找到, 无法安装 Hanafubuki 启动器, 请检查 ComfyUI 是否已正确安装, 或者尝试运行 ComfyUI Installer 进行修复`" -Level ERROR
        return
    }

    `$installed = Install-HanafubukiPackage -InstallRoot `$env:COMFYUI_INSTALLER_ROOT -CachePath `$env:CACHE_HOME
    if (`$installed) {
        Move-PortableLauncherRuntime -InstallRoot `$env:COMFYUI_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"Hanafubuki 启动器`" | Out-Null
    }
}
...
更多帮助信息可在 ComfyUI Installer 文档中查看: https://licyk.github.io/sd-webui-all-in-one/installer/comfyui/
`"
}
...
function Main {
    ...
    Write-Log `"更多帮助信息可在 ComfyUI Installer 项目地址查看: https://licyk.github.io/sd-webui-all-in-one/installer/comfyui/`"
}
...
ComfyUI 项目地址：https://github.com/Comfy-Org/ComfyUI
    ...
    if (Test-Path (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX "hanamizuki.exe")) {
        ...
        foreach ($url in $urls) {
            ...
            try {
                ...
                Invoke-WebRequest @web_request_params
                Move-Item -Path (Join-NormalizedPath $env:CACHE_HOME "hanamizuki_tmp.exe") -Destination (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX "hanamizuki.exe") -Force
...
function Use-InstallMode {
    ...
    Write-Log "ComfyUI Installer 使用文档: https://licyk.github.io/sd-webui-all-in-one/installer/comfyui/"
    ...
    Save-InstallResultSnapshot -CliName "comfyui" -WebUIPath (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX)
...
function Use-BuildMode {
    ...
    if ($script:BuildWithUpdate) {
        ...
        . (Join-NormalizedPath $script:InstallPath "update.ps1") @launch_args
    }
