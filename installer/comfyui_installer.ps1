...
运行目录: `$PSScriptRoot
Python 目录: `$(`$(Get-Command python -ErrorAction SilentlyContinue).Source)
Git 目录: `$(`$(Get-Command git -ErrorAction SilentlyContinue).Source)
Aria2 目录: `$(`$(Get-Command aria2c -ErrorAction SilentlyContinue).Source)

ComfyUI Installer 版本: `$script:COMFYUI_INSTALLER_VERSION
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        try {
            Invoke-WithLogging -LogPrefix `"comfyui_launch_log`" -ScriptBlock {
            ...
            Write-Log `"ComfyUI 出现异常, 已退出, 请检查控制台日志`" -Level ERROR
        }
        Read-Host | Out-Null
    }
}
