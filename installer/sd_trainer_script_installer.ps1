...
$script:SD_TRAINER_SCRIPT_INSTALLER_VERSION = 209
...
# 环境变量
$env:SD_SCRIPTS_PATH = "$script:InstallPath/$env:CORE_PREFIX"
$env:SD_SCRIPTS_ROOT = "$script:InstallPath/$env:CORE_PREFIX"
    ...
    ForEach ($url in $urls) {
        ...
        try {
            ...
            if ($i -lt $urls.Length) {
                ...
                Write-Log "Aria2 安装失败, 终止 SD Trainer Script 安装进程, 可尝试重新运行 SD Trainer Script Installer 重试失败的安装" -Level ERROR
                if (!($script:BuildMode)) { Read-Host | Out-Null }
                exit 1
            }
        }
...
function Initialize-EnvPath {
    ...
    `$env:SD_SCRIPTS_ROOT = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
    `$env:SD_TRAINER_SCRIPT_INSTALLER_ROOT = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
...
function Update-Installer {
    ...
    if ((Test-Path `"`$PSScriptRoot/disable_update.txt`") -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 SD Trainer Script Installer 的自动检查更新功能`"
        return
    }
    ...
    ForEach (`$url in `$urls) {
        ...
        try {
            `$web_request_params = @{
                Uri = `$url
                UseBasicParsing = `$true
                OutFile = `"`$env:CACHE_HOME/sd_trainer_script_installer.ps1`"
            }
            Invoke-WebRequest @web_request_params
            `$latest_version = [int]`$(
            ...
            if (`$i -lt `$urls.Length) {
                ...
                Write-Log `"检查 SD Trainer Script Installer 更新失败`"
                return
            }
        }
    }
...
".Trim()
    ...
    if (!(Test-Path "$InstallPath/train.ps1")) {
        ...
        Write-FileWithStreamWriter -Encoding UTF8BOM -Path "$InstallPath/train.ps1" -Value $content
    }
}
...
try {
    ...
    Write-Host `"launch_sd_trainer_script_installer.ps1`" -ForegroundColor Yellow
...
".Trim()
    ...
    Write-Log "$(if (Test-Path "$script:InstallPath/init.ps1") { "更新" } else { "生成" }) init.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path "$script:InstallPath/init.ps1" -Value $content
}
...
function Download-Installer {
    ...
    ForEach (`$url in `$urls) {
        ...
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
            OutFile = `"`$PSScriptRoot/cache/sd_trainer_script_installer.ps1`"
        }
        Invoke-WebRequest @web_request_params
        if (`$?) {
            ...
            Write-Log `"下载 SD Trainer Script Installer 脚本失败`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
...
".Trim()
    ...
    Write-Log "$(if (Test-Path "$script:InstallPath/launch_sd_trainer_script_installer.ps1") { "更新" } else { "生成" }) launch_sd_trainer_script_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path "$script:InstallPath/launch_sd_trainer_script_installer.ps1" -Value $content
}
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"SD Trainer Script`"; p=`"`$env:CORE_PREFIX/main.py`"; t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
`$env:SD_SCRIPTS_PATH = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
