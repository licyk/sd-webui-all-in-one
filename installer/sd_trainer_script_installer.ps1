...
$script:SD_TRAINER_SCRIPT_INSTALLER_VERSION = 374
...
function Main {
    ...
    while (`$true) {
        ...
        `$menu = @(
            ...
            @{ id=13; n=`"补丁系统 Runtime`"; v=`$(Get-ToggleStatus `"enable_hotpatcher_runtime.txt`" `"启用`" `"禁用`") },
        ...
        switch (`$choice) {
            ...
            `"13`" { Set-ToggleSetting `"enable_hotpatcher_runtime.txt`" `"补丁系统 Runtime`" (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `"enable_hotpatcher_runtime.txt`"))) }
            ...
            `"17`" { Start-Process `"https://licyk.github.io/sd-webui-all-in-one/installer/sd-trainer-script/`" }
