...
$script:SD_WEBUI_INSTALLER_VERSION = 453
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 SD WebUI Installer 的自动检查更新功能`"
        return
    }
...
function Update-Hotpatcher-Port {
    Write-Log `"当前 Hotpatcher 端口: `$(Get-TextStatus `"hotpatcher_port.txt`" `"自动`")`"
    Write-Log `"请输入 Hotpatcher runtime 通信端口 (1-65535, 直接回车删除):`"
