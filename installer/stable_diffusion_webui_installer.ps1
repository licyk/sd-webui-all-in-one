...
$script:SD_WEBUI_INSTALLER_VERSION = 407
...
function Get-HotpatcherLaunchArgs {
    ...
    `$config_path = if (`$script:HotpatcherConfig) { `$script:HotpatcherConfig } else { `$default_config_path }

    if ((!(`$script:HotpatcherConfig)) -and (!(Test-Path `$default_config_path))) {
        Write-Log `"未找到 Hotpatcher 默认配置文件, 正在导出默认配置到 `$default_config_path`"
    ...
    if (`$hotpatcher_runtime_enabled) {
        ...
        `$raw_ignored_port = Get-TrimmedTextFile (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`")
        if (`$script:HotpatcherPortProvided -or (-not [string]::IsNullOrWhiteSpace(`$raw_ignored_port))) {
    ...
    if (`$script:HotpatcherPortProvided) {
        `$port = `$script:HotpatcherPort
    } elseif (Test-Path (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`")) {
        `$raw_port = Get-TrimmedTextFile (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`")
        `$parsed_port = 0
        if ([int]::TryParse(`$raw_port, [ref]`$parsed_port)) {
            `$port = `$parsed_port
        } elseif (-not [string]::IsNullOrWhiteSpace(`$raw_port)) {
            Write-Log `"Hotpatcher 端口配置无效, 已忽略: `$raw_port`" -Level WARNING
        }
    }

    if (`$hotpatcher_runtime_enabled -and (`$null -ne `$port)) {
        if ((`$port -ge 1) -and (`$port -le 65535)) {
            `$ArrayList.Add(`"--hotpatcher-port`") | Out-Null
            `$ArrayList.Add(`$port) | Out-Null
        } else {
            Write-Log `"Hotpatcher 端口超出范围, 已忽略: `$port`" -Level WARNING
        }
    }
...
function Get-LaunchCoreArgs {
    ...
    if (!(`$script:BuildMode)) {
        ...
        Get-HotpatcherLaunchArgs `$launch_params
...
- version_manager.ps1：管理 Stable Diffusion WebUI / 扩展的版本；安装，启用 / 禁用，卸载扩展。
