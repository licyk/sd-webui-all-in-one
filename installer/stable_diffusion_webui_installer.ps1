...
"@)][string]$InstallPythonVersion,
    ...
    [Parameter(HelpMessage=@"
启用 Stable Diffusion WebUI Installer 快照重建模式, 将根据快照文件重新准备 Python 版本并恢复环境
...
"@)][switch]$DisableAutoMirror,
    ...
    [Parameter(HelpMessage=@"
禁用 SD WebUI Installer 使用 PyPI 软件包镜像源, 使用 PyPI 官方源下载 Python 软件包
"@)][switch]$DisableProxy,
...
"@)][switch]$DisableUpdate,
    ...
    [Parameter(HelpMessage=@"
(仅在 SD WebUI Installer 构建模式下生效, 并且只作用于 SD WebUI Installer 管理脚本) 禁用 Hugging Face 镜像源, 不使用 Hugging Face 镜像源下载文件
"@)][switch]$DisableHuggingFaceMirror,

    [Parameter(HelpMessage=@"
(仅在 SD WebUI Installer 构建模式下生效, 并且只作用于 SD WebUI Installer 管理脚本) 使用自定义 Hugging Face 镜像源地址, 例如代理服务器地址为 https://hf-mirror.com, 则使用 -UseCustomHuggingFaceMirror "https://hf-mirror.com" 设置 Hugging Face 镜像源地址
...
"@)][string]$LaunchArg,
    ...
    [Parameter(HelpMessage=@"
(仅在 SD WebUI Installer 构建模式下生效, 并且只作用于 SD WebUI Installer 管理脚本) 禁用 Hotpatcher 补丁系统注入
"@)][switch]$DisableHotpatcher,

    [Parameter(HelpMessage=@"
(仅在 SD WebUI Installer 构建模式下生效, 并且只作用于 SD WebUI Installer 管理脚本) 设置 Hotpatcher runtime 通信端口, 范围为 1 到 65535
"@)][int]$HotpatcherPort,
...
"@)][switch]$EnableHotpatcherRuntime,
    ...
    [Parameter(HelpMessage=@"
(仅在 SD WebUI Installer 构建模式下生效, 并且只作用于 SD WebUI Installer 管理脚本) 创建 Stable Diffusion WebUI 启动创建启动快捷方式
...
"@)][switch]$DisableCUDAMalloc,
    ...
    [Parameter(HelpMessage=@"
(仅在 SD WebUI Installer 构建模式下生效, 且只作用于 SD WebUI Installer 管理脚本) 禁用 SD WebUI Installer 检查 Stable Diffusion WebUI 运行环境中存在的问题, 禁用后可能会导致 Stable Diffusion WebUI 环境中存在的问题无法被发现并修复
...
if (-not $script:InstallPath) {
    $script:InstallPath = Join-NormalizedPath $PSScriptRoot "stable-diffusion-webui"
}
...
$script:SD_WEBUI_INSTALLER_VERSION = 505
...
# 环境变量
$env:SD_WEBUI_PATH = Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX
$env:SD_WEBUI_ROOT = Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX
...
$env:SD_WEBUI_ALL_IN_ONE_LOGGER_NAME = "SD WebUI Installer"
...
function Write-Log {
    ...
    param(
        ...
        [string]$Level = "INFO",
        [string]$Name = "SD WebUI Installer"
    )
...
function Get-Version {
    $ver = $([string]$script:SD_WEBUI_INSTALLER_VERSION).ToCharArray()
    $major = ($ver[0..($ver.Length - 3)])
    $minor = $ver[-2]
    $micro = $ver[-1]
    Write-Log "SD WebUI Installer 版本: v${major}.${minor}.${micro}"
}
...
function Get-InstallBranch {
    $branch_mapping_table = @(
        ...
        @{ Key = "sd_next_dev";           Val = "sd_next_dev" }
    )
    $target_branch = $null
...
function Get-LaunchCoreArgs {
    ...
    if ($script:NoPreDownloadExtension) {
...
function Install-ArchiveResource {
    ...
    if (-not $success) {
        Write-Log "$ResourceName 安装失败, 终止安装进程, 可尝试重新运行 SD WebUI Installer 重试失败的安装" -Level ERROR
        if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
        exit 1
    }
...
function Test-InstallerGitKernelSnapshot {
    ...
    if ($null -eq $Snapshot.kernel) {
        Stop-SnapshotRestore "快照缺少内核信息, 无法在 Installer 重建模式中恢复 Stable Diffusion WebUI"
    }
    if (-not [System.Convert]::ToBoolean($Snapshot.kernel.is_git_repo)) {
        Stop-SnapshotRestore "快照中的 Stable Diffusion WebUI 内核不是 Git 仓库, Installer 重建模式无法恢复该内核"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.url)) {
        Stop-SnapshotRestore "快照中的 Stable Diffusion WebUI 内核缺少 Git 远程地址, 无法从头重建环境"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.commit)) {
        Stop-SnapshotRestore "快照中的 Stable Diffusion WebUI 内核缺少 Git commit, 无法恢复到快照版本"
    }
}
...
function Resolve-SnapshotRebuildConfig {
    ...
    if ($snapshot_webui_type -ne "sd_webui") {
        Stop-SnapshotRestore "快照 WebUI 类型不匹配: 期望 'sd_webui', 实际 '$snapshot_webui_type'"
    }
...
function Install-Git {
    ...
    elseif ($platform -eq "linux") {
        ...
        try {
            ...
            Write-Log "安装 Git 失败, 终止安装进程, 可尝试重新运行 SD WebUI Installer 重试失败的安装" -Level ERROR
            if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
            exit 1
        }
    }
...
function Invoke-Installation {
    ...
    if ($script:RestoreFromSnapshot) {
        ...
        $snapshot_path = Get-NormalizedFilePath $script:SnapshotPath
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "sd-webui", "restore", $snapshot_path)
        & python -m sd_webui_all_in_one sd-webui restore $snapshot_path $launch_params
        $operation_label = "恢复"
    } else {
        $launch_params = Get-LaunchCoreArgs
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "sd-webui", "install")
    ...
    if ($exit_code -ne 0) {
        ...
        Write-Log "运行 SD WebUI All In One ${operation_label} Stable Diffusion WebUI 时发生了错误, 终止 Stable Diffusion WebUI ${operation_label}进程, 可尝试重新运行 Stable Diffusion WebUI Installer 重试失败的操作" -Level ERROR
        if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
        exit 1
    }
...
# SD WebUI Installer 版本和检查更新间隔
`$script:SD_WEBUI_INSTALLER_VERSION = $script:SD_WEBUI_INSTALLER_VERSION
...
function Initialize-EnvPath {
    ...
    `$env:SD_WEBUI_ROOT = Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 SD WebUI Installer 的自动检查 Installer 更新功能`"
        return
    }

    if (`$script:BuildMode) {
        Write-Log `"SD WebUI Installer 构建模式已启用, 跳过 SD WebUI Installer 更新检查`"
        return
    }
    ...
    foreach (`$url in `$urls) {
        ...
        try {
            ...
            Invoke-WebRequest @web_request_params
            `$latest_version = [int]`$(
                Get-Content (Join-NormalizedPath `$env:CACHE_HOME `"stable_diffusion_webui_installer.ps1`") -Encoding UTF8 |
            ...
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试检查 SD WebUI Installer 更新中`" -Level WARNING
            } else {
                Write-Log `"检查 SD WebUI Installer 更新失败`" -Level ERROR
                return
            }
        }
    }

    if (`$latest_version -le `$script:SD_WEBUI_INSTALLER_VERSION) {
        Write-Log `"SD WebUI Installer 已是最新版本`"
        return
    }

    Write-Log `"调用 SD WebUI Installer 进行更新中`"
    & (Join-NormalizedPath `$env:CACHE_HOME `"stable_diffusion_webui_installer.ps1`") -InstallPath `$PSScriptRoot -UseUpdateMode
    ...
    Write-Log `"更新结束, 重新启动 SD WebUI Installer 管理脚本中, 使用的命令行参数: `$raw_params`"
...
function Test-PythonAndGit {
    ...
    if (`$python_cmd) {
        ...
        if (-not ((`$python_cmd) -and ((`$python_cmd.ToString().StartsWith(`$python_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$python_cmd.ToString().StartsWith(`$python_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
            Write-Log `"检测到当前使用的 Python 路径为 `${python_cmd}, 但未在 `${python_path_prefix} 或 `${python_extra_path_prefix} 这两个受 SD WebUI Installer 管理的 Python 路径, 即当前正在使用外部的 Python 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_stable_diffusion_webui_installer.ps1 修复运行环境`" -Level ERROR
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Python, 这将导致运行时发生异常, 请运行 launch_stable_diffusion_webui_installer.ps1 修复运行环境`" -Level ERROR
    }
    ...
    if (`$git_cmd) {
        if ((Get-CurrentPlatform) -eq `"windows`") {
            ...
            if (-not ((`$git_cmd) -and ((`$git_cmd.ToString().StartsWith(`$git_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$git_cmd.ToString().StartsWith(`$git_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
                Write-Log `"检测到当前使用的 Git 路径为 `${git_cmd}, 但未在 `${git_path_prefix} 或 `${git_extra_path_prefix} 这两个受 SD WebUI Installer 管理的 Git 路径, 即当前正在使用外部的 Git 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_stable_diffusion_webui_installer.ps1 修复运行环境`" -Level ERROR
            }
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Git, 这将导致运行时发生异常, 请运行 launch_stable_diffusion_webui_installer.ps1 修复运行环境`" -Level ERROR
    }
}
...
function Add-Shortcut {
    ...
    Write-Log `"检测到 enable_shortcut.txt 配置文件 / -EnableShortcut 命令行参数, 开始检查 Stable Diffusion WebUI 快捷启动方式中`"
...
function Test-WebUIEnv {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 Stable Diffusion WebUI 启动前环境检测, 这可能会导致 Stable Diffusion WebUI 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
`"@)][switch]`$EnableHotpatcherRuntime,
    ...
    [Parameter(HelpMessage=@`"
创建 Stable Diffusion WebUI 启动创建启动快捷方式
`"@)][switch]`$EnableShortcut,
...
function Main {
    ...
    if (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX))) {
        Write-Log `"内核路径 `$(Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX) 未找到, 请检查 Stable Diffusion WebUI 是否已正确安装, 或者尝试运行 SD WebUI Installer 进行修复`" -Level ERROR
    ...
    if (`$script:BuildMode) {
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"check-env`")
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"launch`")
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"update`")
...
# 更新扩展脚本
function Write-UpdateExtensionScript {
    $content = "
param (
    [Parameter(HelpMessage=@`"
获取 SD WebUI Installer 的帮助信息
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"extension`", `"update`")
    Set-SnapshotCliArgs `$launch_args
...
function Download-Installer {
    ...
    foreach (`$url in `$urls) {
        ...
        try {
            Invoke-WebRequest @web_request_params
            Write-Log `"下载 SD WebUI Installer 脚本成功`"
            break
        } catch {
            Write-Log `"下载 SD WebUI Installer 脚本失败: `$(`$_.Exception.Message)`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试下载 SD WebUI Installer 脚本`" -Level WARNING
            } else {
                Write-Log `"下载 SD WebUI Installer 脚本失败, 可尝试重新运行 SD WebUI Installer 下载脚本`" -Level ERROR
                return `$false
            }
        }
    }
...
function Main {
    ...
    if (`$status) {
        ...
        try {
            ...
            Write-Log `"运行 SD WebUI Installer 时出现了错误: `$_`" -Level ERROR
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"reinstall-pytorch`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"model`", `"install-library`")
...
function Main {
    ...
    if (!(Test-Path (Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX))) {
        Write-Log `"内核路径 `$(Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX) 未找到, 请检查 Stable Diffusion WebUI 是否已正确安装, 或者尝试运行 Stable Diffusion WebUI Installer 进行修复`" -Level ERROR
        Exit-ManagerScript -ExitCode 1
    }

    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"gui`", `"version-manager`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"sd-webui`", `"gui`", `"snapshot-manager`")
    & python -m sd_webui_all_in_one sd-webui gui snapshot-manager `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }

    Write-Log `"退出 Stable Diffusion WebUI 快照管理脚本`"
...
function Update-Hotpatcher-Port {
    Write-Log `"当前 Hotpatcher 运行时通信端口: `$(Get-TextStatus `"hotpatcher_port.txt`" `"自动`")`"
    `$current_port = Get-TextStatus `"hotpatcher_port.txt`" `$null
    `$input_port = Read-SingleLineSettingEditor -Prompt `"Hotpatcher 运行时通信端口 (1-65535, 清空后保存可删除):`" -InitialText `$current_port
    if (`$null -eq `$input_port) { Write-Log `"已取消修改 Hotpatcher 运行时通信端口`"; return }
    if (!(`$input_port)) {
    ...
    `$port = 0
    if (([int]::TryParse(`$input_port, [ref]`$port)) -and (`$port -ge 1) -and (`$port -le 65535)) {
        Write-FileWithStreamWriter -Path (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`") -Value `$port -Encoding UTF8
        Write-Log `"Hotpatcher 运行时通信端口设置已保存`"
    } else {
        Write-Log `"端口无效, 请输入 1 到 65535 的整数`" -Level WARNING
    }
}
...
function Main {
    ...
    while (`$true) {
        Write-Log `"=== Stable Diffusion WebUI 本地设置菜单 ===`"
        `$menu = @(
            ...
            @{ id=10; n=`"Hotpatcher 运行时端口`"; v=`$(Get-TextStatus `"hotpatcher_port.txt`" `"自动`") },
        ...
        switch (`$choice) {
            ...
            `"19`" { Start-Process `"https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/`" }
...
function global:Install-Hanamizuki {
    ...
    if (!(Test-Path (Join-NormalizedPath `$env:SD_WEBUI_INSTALLER_ROOT `$env:CORE_PREFIX))) {
        Write-Log `"内核路径 (Join-NormalizedPath `$env:SD_WEBUI_INSTALLER_ROOT `$env:CORE_PREFIX) 未找到, 无法安装绘世启动器, 请检查 Stable Diffusion WebUI 是否已正确安装, 或者尝试运行 SD WebUI Installer 进行修复`" -Level ERROR
        return
    }
...
这是 Stable Diffusion WebUI 的安装后速查说明。完整安装、启动、模型、扩展、更新和故障排查说明请阅读文档站。
SD WebUI Installer 文档：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/

一、安装完成后先做什么
1. 运行 launch.ps1 启动 Stable Diffusion WebUI。启动成功后，浏览器会打开或终端会显示访问地址。
2. 需要模型时运行 download_models.ps1；模型放置位置和推荐资源见模型与资源文档。
3. 需要更新 WebUI 时运行 update.ps1；需要更新扩展时运行 update_extension.ps1。
4. 需要切换 WebUI 分支时运行 switch_branch.ps1。
5. 需要执行 Python、Pip、Git 命令时运行 terminal.ps1，或先运行 activate.ps1 激活环境。

二、常用脚本速查
- launch.ps1：启动 Stable Diffusion WebUI。
- update.ps1：更新 Stable Diffusion WebUI 和管理脚本。
- update_extension.ps1：更新 Stable Diffusion WebUI 扩展。
- download_models.ps1：下载模型到 WebUI 的模型目录。
- reinstall_pytorch.ps1：PyTorch 损坏、版本不匹配或需要切换 CUDA / ROCm / XPU 版本时使用。
- switch_branch.ps1：切换 Stable Diffusion WebUI、Forge、reForge、Forge Classic、AMDGPU、SD.Next 等分支。
- version_manager.ps1：管理 WebUI 和扩展的版本、安装、启用、禁用和卸载。
...
- launch_stable_diffusion_webui_installer.ps1：重新获取并运行最新 SD WebUI Installer。
...
- git：安装器自带 Git。
- core：Stable Diffusion WebUI 程序和主要数据目录。

四、遇到问题
- Windows 下运行 .ps1 后立刻闪退：先运行 configure_env.bat，再重新运行原脚本。
- 启动、安装或更新报错：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/troubleshooting/
- 不确定脚本参数：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/commands/
- 下载器或 Launcher 出错：https://licyk.github.io/sd-webui-all-in-one/tools/troubleshooting/

五、详细文档
- 启动与使用：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/usage/
- 模型与资源：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/resources/
- 配置与镜像：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/config/
- 维护与迁移：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/maintenance/
- 常用命令：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/commands/
...
支持的 WebUI 分支列表：https://licyk.github.io/sd-webui-all-in-one/installer/sd-webui/
...
function Use-InstallMode {
    ...
    Save-InstallResultSnapshot -CliName "sd-webui" -WebUIPath (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX)
