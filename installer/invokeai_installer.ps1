...
"@)][string]$InstallPythonVersion,
    ...
    [Parameter(HelpMessage=@"
启用 InvokeAI Installer 快照重建模式, 将根据快照文件重新准备 Python 版本并恢复环境
...
"@)][switch]$UseUpdateMode,
    ...
    [Parameter(HelpMessage=@"
禁用 InvokeAI Installer 使用 PyPI 软件包镜像源, 使用 PyPI 官方源下载 Python 软件包
...
"@)][switch]$DisableUpdate,
    ...
    [Parameter(HelpMessage=@"
(仅在 InvokeAI Installer 构建模式下生效, 并且只作用于 InvokeAI Installer 管理脚本) 禁用 Hugging Face 镜像源, 不使用 Hugging Face 镜像源下载文件
"@)][switch]$DisableHuggingFaceMirror,

    [Parameter(HelpMessage=@"
(仅在 InvokeAI Installer 构建模式下生效, 并且只作用于 InvokeAI Installer 管理脚本) 使用自定义 Hugging Face 镜像源地址, 例如代理服务器地址为 https://hf-mirror.com, 则使用 -UseCustomHuggingFaceMirror "https://hf-mirror.com" 设置 Hugging Face 镜像源地址
...
"@)][switch]$EnableHotpatcherRuntime,
    ...
    [Parameter(HelpMessage=@"
(仅在 InvokeAI Installer 构建模式下生效, 并且只作用于 InvokeAI Installer 管理脚本) 创建 InvokeAI 启动创建启动快捷方式
...
$script:INVOKEAI_INSTALLER_VERSION = 512
    ...
    foreach ($runtime_name in $runtime_names) {
        ...
        if (!(Test-Path -LiteralPath $source_path -PathType Container)) {
            if (($runtime_name -eq "python") -or ((Get-CurrentPlatform) -eq "windows")) {
                Write-Log "环境缺少 $display_name, 无法为 $LauncherName 准备 $display_name 环境, 请重新运行 InvokeAI Installer 修复环境" -Level WARNING
                $runtime_ready = $false
            }
            continue
        }
...
function Resolve-SnapshotRebuildConfig {
    ...
    if ($snapshot_webui_type -ne "invokeai") {
        Stop-SnapshotRestore "快照 WebUI 类型不匹配: 期望 'invokeai', 实际 '$snapshot_webui_type'"
    }
...
function Invoke-Installation {
    ...
    if ($script:RestoreFromSnapshot) {
        ...
        $snapshot_path = Get-NormalizedFilePath $script:SnapshotPath
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "invokeai", "restore", $snapshot_path)
        & python -m sd_webui_all_in_one invokeai restore $snapshot_path $launch_params
        $operation_label = "恢复"
    } else {
        $launch_params = Get-LaunchCoreArgs
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "invokeai", "install")
    ...
    if ($exit_code -ne 0) {
        ...
        Write-Log "运行 SD WebUI All In One ${operation_label} InvokeAI 时发生了错误, 终止 InvokeAI ${operation_label}进程, 可尝试重新运行 InvokeAI Installer 重试失败的操作" -Level ERROR
        if ((-not $script:BuildMode) -and (-not $script:NoPause)) { Read-Host | Out-Null }
        exit 1
    }
...
function Update-Installer {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_update.txt`")) -or (`$script:DisableUpdate)) {
        Write-Log `"检测到 disable_update.txt 更新配置文件 / -DisableUpdate 命令行参数, 已禁用 InvokeAI Installer 的自动检查 Installer 更新功能`"
        return
    }
    ...
    foreach (`$url in `$urls) {
        ...
        try {
            ...
            if (`$i -lt `$urls.Length) {
                ...
                Write-Log `"检查 InvokeAI Installer 更新失败`" -Level ERROR
                return
            }
        }
    }
...
function Test-PythonAndGit {
    ...
    if (`$python_cmd) {
        ...
        if (-not ((`$python_cmd) -and ((`$python_cmd.ToString().StartsWith(`$python_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$python_cmd.ToString().StartsWith(`$python_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
            Write-Log `"检测到当前使用的 Python 路径为 `${python_cmd}, 但未在 `${python_path_prefix} 或 `${python_extra_path_prefix} 这两个受 InvokeAI Installer 管理的 Python 路径, 即当前正在使用外部的 Python 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_invokeai_installer.ps1 修复运行环境`" -Level ERROR
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Python, 这将导致运行时发生异常, 请运行 launch_invokeai_installer.ps1 修复运行环境`" -Level ERROR
    }
    ...
    if (`$git_cmd) {
        if ((Get-CurrentPlatform) -eq `"windows`") {
            ...
            if (-not ((`$git_cmd) -and ((`$git_cmd.ToString().StartsWith(`$git_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$git_cmd.ToString().StartsWith(`$git_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
                Write-Log `"检测到当前使用的 Git 路径为 `${git_cmd}, 但未在 `${git_path_prefix} 或 `${git_extra_path_prefix} 这两个受 InvokeAI Installer 管理的 Git 路径, 即当前正在使用外部的 Git 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_invokeai_installer.ps1 修复运行环境`" -Level ERROR
            }
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Git, 这将导致运行时发生异常, 请运行 launch_invokeai_installer.ps1 修复运行环境`" -Level ERROR
    }
}
...
function Test-WebUIEnv {
    ...
    if ((Test-Path (Join-NormalizedPath `$PSScriptRoot `"disable_check_env.txt`")) -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 InvokeAI 启动前环境检测, 这可能会导致 InvokeAI 运行环境中存在的问题无法被发现并解决`" -Level WARNING
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"invokeai`", `"check-env`")
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"invokeai`", `"launch`")
...
function Download-Installer {
    ...
    foreach (`$url in `$urls) {
        ...
        try {
            ...
            Write-Log `"下载 InvokeAI Installer 脚本失败: `$(`$_.Exception.Message)`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
...
function Main {
    ...
    if (`$status) {
        ...
        try {
            ...
            Write-Log `"运行 InvokeAI Installer 时出现了错误: `$_`" -Level ERROR
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"invokeai`", `"model`", `"install-library`")
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"invokeai`", `"gui`", `"snapshot-manager`")
    & python -m sd_webui_all_in_one invokeai gui snapshot-manager `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }

    Write-Log `"退出 InvokeAI 快照管理脚本`"
...
function Update-Hotpatcher-Port {
    ...
    Write-Log `"1. 配置端口 | 2. 删除端口配置 | 3. 返回`"
    `$choice = Get-UserInput
    if (`$choice -eq `"1`") {
        `$current_port = Get-TextStatus `"hotpatcher_port.txt`" `$null
        `$port_text = Read-SingleLineSettingEditor -Prompt `"Hotpatcher 运行时通信端口 (1..65535):`" -InitialText `$current_port
        ...
        if (([int]::TryParse(`$port_text, [ref]`$port)) -and (`$port -ge 1) -and (`$port -le 65535)) {
            Set-Content -Path (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`") -Value `$port -Encoding UTF8
            Write-Log `"Hotpatcher 运行时通信端口设置已保存`"
        } else {
            Write-Log `"Hotpatcher 端口必须在 1..65535 范围内`" -Level WARNING
        }
    }
    elseif (`$choice -eq `"2`") {
        Remove-Item (Join-NormalizedPath `$PSScriptRoot `"hotpatcher_port.txt`") -Force -ErrorAction SilentlyContinue
        Write-Log `"Hotpatcher 端口配置已删除`"
    }
}
...
function global:Install-Hanafubuki {
    `$core_path = Join-NormalizedPath `$env:INVOKEAI_INSTALLER_ROOT `$env:CORE_PREFIX
    if (!(Test-Path -LiteralPath `$core_path -PathType Container)) {
        Write-Log `"内核路径 `$core_path 未找到, 无法安装 Hanafubuki 启动器, 请检查 InvokeAI 是否已正确安装, 或者尝试运行 InvokeAI Installer 进行修复`" -Level ERROR
        return
    }

    `$installed = Install-HanafubukiPackage -InstallRoot `$env:INVOKEAI_INSTALLER_ROOT -CachePath `$env:CACHE_HOME
    if (`$installed) {
        Move-PortableLauncherRuntime -InstallRoot `$env:INVOKEAI_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"Hanafubuki 启动器`" | Out-Null
    }
}
...
这是 InvokeAI 的安装后速查说明。完整安装、启动、模型、节点、更新和故障排查说明请阅读文档站。
InvokeAI Installer 文档：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/

一、安装完成后先做什么
1. 运行 launch.ps1 启动 InvokeAI。启动成功后，浏览器会打开或终端会显示访问地址。
2. 需要模型时运行 download_models.ps1；模型放置位置和推荐资源见模型与资源文档。
3. 需要更新 InvokeAI 时运行 update.ps1；需要更新自定义节点时运行 update_node.ps1。
4. 需要执行 Python、Pip、InvokeAI 命令时运行 terminal.ps1，或先运行 activate.ps1 激活环境。

二、常用脚本速查
- launch.ps1：启动 InvokeAI。
- update.ps1：更新 InvokeAI 和管理脚本。
- update_node.ps1：更新 InvokeAI 自定义节点。
- download_models.ps1：下载模型到 models 目录。
- reinstall_pytorch.ps1：PyTorch 损坏、版本不匹配或需要切换 CUDA / ROCm / XPU 版本时使用。
- version_manager.ps1：管理 InvokeAI 和自定义节点的版本、安装、启用、禁用和卸载。
...
- launch_invokeai_installer.ps1：重新获取并运行最新 InvokeAI Installer。
- configure_env.bat：修复 Windows 运行 .ps1 后立刻闪退、PowerShell 脚本运行限制和长路径支持问题。

三、目录说明
- cache：Pip / Hugging Face 等缓存目录。
- python：安装器自带 Python 和 InvokeAI 程序。请勿添加到系统环境变量。
- git：安装器自带 Git。
- core：InvokeAI 的模型、图片、配置文件等数据目录。

四、遇到问题
- Windows 下运行 .ps1 后立刻闪退：先运行 configure_env.bat，再重新运行原脚本。
- 启动、安装或更新报错：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/troubleshooting/
- 不确定脚本参数：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/commands/
- 下载器或 Launcher 出错：https://licyk.github.io/sd-webui-all-in-one/tools/troubleshooting/

五、详细文档
- 启动与使用：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/usage/
- 模型与资源：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/resources/
- 配置与镜像：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/config/
- 维护与迁移：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/maintenance/
- 常用命令：https://licyk.github.io/sd-webui-all-in-one/installer/invokeai/commands/
...
function Use-InstallMode {
    ...
    Save-InstallResultSnapshot -CliName "invokeai" -WebUIPath (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX)
