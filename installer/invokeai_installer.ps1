...
$script:INVOKEAI_INSTALLER_VERSION = 295
...
function Invoke-Installation {
    ...
    & python -m sd_webui_all_in_one.cli_manager.main invokeai install $launch_params
    if (!($?)) {
...
try {
    ...
    Write-Host `"launch_invokeai_installer.ps1`" -ForegroundColor Yellow
...
function Add-Shortcut {
    ...
    if (!(Test-Path `"`$shortcut_icon`")) {
        ...
        Invoke-WebRequest @web_request_params
        if (!(`$?)) {
            Write-Log `"获取 InvokeAI 图标失败, 无法创建 InvokeAI 快捷启动方式`"
            return
        }
    }

    Write-Log `"更新 InvokeAI 快捷启动方式`"
...
function Test-WebUIEnv {
    ...
    if ((Test-Path `"`$PSScriptRoot/disable_check_env.txt`") -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 InvokeAI 运行环境检测, 这可能会导致 InvokeAI 运行环境中存在的问题无法被发现并解决`"
...
function Main {
    ...
    if (!(Test-Path `"`$PSScriptRoot/`$env:CORE_PREFIX`")) {
        Write-Log `"内核路径 `$PSScriptRoot\`$env:CORE_PREFIX 未找到, 请检查 InvokeAI 是否已正确安装, 或者尝试运行 InvokeAI Installer 进行修复`"
        Read-Host | Out-Null
        return
    }
...
try {
    `$global:OriginalScriptPath = `$PSCommandPath
    `$global:LaunchCommandLine = `$MyInvocation.Line
    (Import-Module `"`$PSScriptRoot/modules.psm1`" -Function `"Initialize-EnvPath`", `"Write-Log`", `"Set-CorePrefix`", `"Get-Version`", `"Update-Installer`", `"Set-Proxy`", `"Update-SDWebUiAllInOne`", `"Set-uv`", `"Set-PyPIMirror`" -PassThru -Force -ErrorAction Stop).Invoke({
        `$script:OriginalScriptPath = `$global:OriginalScriptPath
        `$script:LaunchCommandLine = `$global:LaunchCommandLine
        Remove-Variable OriginalScriptPath -Scope Global -Force
        Remove-Variable LaunchCommandLine -Scope Global -Force
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main invokeai update `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main invokeai custom-node update `$launch_args
...
".Trim()
    ...
    Write-Log "$(if (Test-Path "$script:InstallPath/launch_invokeai_installer.ps1") { "更新" } else { "生成" }) launch_invokeai_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path "$script:InstallPath/launch_invokeai_installer.ps1" -Value $content
}
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main invokeai reinstall-pytorch `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main invokeai model install-library `$launch_args
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"InvokeAI`"; p=`"`$env:CORE_PREFIX/main.py`"; t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
function Main {
    ...
    if (Test-Path `"`$env:INVOKEAI_INSTALLER_ROOT/`$env:CORE_PREFIX/python/python.exe`") {
        `$env:UV_PYTHON = `"`$env:INVOKEAI_INSTALLER_ROOT/`$env:CORE_PREFIX/python/python.exe`"
    }
...
function Get-InstallerCmdletHelp {
    $content = "
使用:
    .\$($script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-InstallPath <安装 InvokeAI 的绝对路径>] [-PyTorchMirrorType <PyTorch 镜像源类型>] [-UseUpdateMode] [-DisablePyPIMirror] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableUV] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像站地址>] [-BuildMode] [-BuildWithUpdate] [-BuildWithUpdateNode] [-BuildWithLaunch] [-BuildWithTorch <PyTorch 版本编号>] [-BuildWitchModel <模型编号列表>] [-NoPreDownloadNode] [-NoPreDownloadModel] [-PyTorchPackage <PyTorch 软件包>] [-NoCleanCache] [-xFormersPackage <xFormers 软件包>] [-DisableUpdate] [-DisableHuggingFaceMirror] [-UseCustomHuggingFaceMirror <HuggingFace 镜像源地址>] [-LaunchArg <InvokeAI 启动参数>] [-EnableShortcut] [-DisableCUDAMalloc] [-DisableEnvCheck]
