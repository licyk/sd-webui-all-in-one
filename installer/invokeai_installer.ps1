...
$script:INVOKEAI_INSTALLER_VERSION = 302
...
function Add-Shortcut {
    ...
    `$shortcut_icon = Join-NormalizedPath `$PSScriptRoot `"invokeai_icon.ico`"
    ...
    if (!(Test-Path `"`$shortcut_icon`")) {
        ...
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
            OutFile = (Join-NormalizedPath `$PSScriptRoot `"invokeai_icon.ico`")
        }
        Invoke-WebRequest @web_request_params
        if (!(`$?)) {
...
function Main {
    ...
    if (`$status) {
        ...
        try {
            Invoke-Expression `"& ```"(Join-NormalizedPath `$PSScriptRoot 'cache' 'invokeai_installer.ps1')```" `$extra_args @arg`"
        }
        catch {
...
function Main {
    ...
    if (Test-Path (Join-NormalizedPath `$env:INVOKEAI_INSTALLER_ROOT `$env:CORE_PREFIX `"python`" `"python.exe`")) {
        `$env:UV_PYTHON = Join-NormalizedPath `$env:INVOKEAI_INSTALLER_ROOT `$env:CORE_PREFIX `"python`" `"python.exe`"
    }
