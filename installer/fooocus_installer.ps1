...
$script:FOOOCUS_INSTALLER_VERSION = 213
...
# 环境变量
$env:FOOOCUS_PATH = "$script:InstallPath/$env:CORE_PREFIX"
$env:FOOOCUS_ROOT = "$script:InstallPath/$env:CORE_PREFIX"
...
function Initialize-EnvPath {
    ...
    `$env:FOOOCUS_ROOT = `"`$PSScriptRoot/`$env:CORE_PREFIX`"
    ...
    ForEach (`$url in `$urls) {
        ...
        try {
            `$web_request_params = @{
                Uri = `$url
                UseBasicParsing = `$true
                OutFile = `"`$env:CACHE_HOME/fooocus_installer.ps1`"
            }
            Invoke-WebRequest @web_request_params
            `$latest_version = [int]`$(
...
function Test-WebUIEnv {
    ...
    if ((Test-Path `"`$PSScriptRoot/disable_check_env.txt`") -or (`$script:DisableEnvCheck)) {
        Write-Log `"检测到 disable_check_env.txt 配置文件 / -DisableEnvCheck 命令行参数, 已禁用 Fooocus 运行环境检测, 这可能会导致 Fooocus 运行环境中存在的问题无法被发现并解决`"
