...
$script:FOOOCUS_INSTALLER_VERSION = 287
...
Fooocus Installer 版本: `$script:FOOOCUS_INSTALLER_VERSION
...
function Get-InstallerCmdletHelp {
    `$content = `"
使用:
    ./`$(`$script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-BuildMode] [-DisablePyPIMirror] [-DisableUpdate] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableHuggingFaceMirror] [-UseCustomHuggingFaceMirror <HuggingFace 镜像源地址>] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像源地址>] [-DisableUV] [-LaunchArg <Fooocus 启动参数>] [-EnableShortcut] [-DisableCUDAMalloc] [-DisableEnvCheck]
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        try {
            Invoke-WithLogging -LogPrefix `"fooocus_launch_log`" -ScriptBlock {
            ...
            Write-Log `"Fooocus 出现异常, 已退出, 请检查控制台日志`" -Level ERROR
        }
        Read-Host | Out-Null
    }
}
...
function Get-InstallerCmdletHelp {
    `$content = `"
使用:
    ./`$(`$script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-BuildMode] [-BuildWithBranch <Fooocus 分支编号>] [-DisableUpdate] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像源地址>]
...
更多的帮助信息请阅读 Fooocus Installer 使用文档: https://github.com/licyk/sd-webui-all-in-one/blob/main/docs/sd_trainer_installer.md
`".Trim()
...
function Main {
    ...
    while (`$true) {
        ...
        switch (`$choice) {
            ...
            `"13`" { Start-Process `"https://github.com/licyk/sd-webui-all-in-one/blob/main/docs/fooocus_installer.md`" }
...
function Get-InstallerCmdletHelp {
    $content = "
使用:
    ./$($script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-InstallPath <安装 Fooocus 的绝对路径>] [-PyTorchMirrorType <PyTorch 镜像源类型>] [-InstallPythonVersion <Python 版本>] [-UseUpdateMode] [-DisablePyPIMirror] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableUV] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像站地址>] [-InstallBranch <安装的 Fooocus 分支>] [-BuildMode] [-BuildWithTorch <PyTorch 版本编号>] [-BuildWithTorchReinstall] [-BuildWithModel <模型编号列表>] [-BuildWithBranch <Fooocus 分支编号>] [-BuildWithUpdate] [-BuildWithLaunch] [-NoPreDownloadModel] [-PyTorchPackage <PyTorch 软件包>] [-xFormersPackage <xFormers 软件包>] [-InstallHanamizuki] [-NoCleanCache] [-DisableUpdate] [-DisableHuggingFaceMirror] [-UseCustomHuggingFaceMirror <HuggingFace 镜像源地址>] [-LaunchArg <Fooocus 启动参数>] [-EnableShortcut] [-DisableCUDAMalloc] [-DisableEnvCheck]
...
参数:
    ...
    -InstallPath <安装 Fooocus 的绝对路径>
        ...
        例如: ./$($script:MyInvocation.MyCommand.Name) -InstallPath `"D:\Download`", 这将指定 Fooocus Installer 安装 Fooocus 到 D:\Download 这个路径
    ...
    -InstallBranch <安装的 Fooocus 分支>
        ...
        例如: ./$($script:MyInvocation.MyCommand.Name) -InstallBranch `"fooocus_main`", 这将指定 Fooocus Installer 安装 lllyasviel - Fooocus 分支
