...
$script:FOOOCUS_INSTALLER_VERSION = 221
...
function Invoke-Installation {
    ...
    & python -m sd_webui_all_in_one.cli_manager.main fooocus install $launch_params
    if (!($?)) {
    ...
    $target_branch = Get-InstallBranch
    $launch_args_map = @{
        "fooocus_main"          = "--language zh --preset fooocus_installer --disable-offload-from-vram --disable-analytics --always-download-new-model"
        "ruined_fooocus_main"   = "--language zh --preset fooocus_installer --disable-offload-from-vram --disable-analytics --always-download-new-model"
        "fooocus_mre_main"      = "--language zh --preset fooocus_installer --disable-offload-from-vram --disable-analytics --always-download-new-model"
    }
...
try {
    ...
    Write-Host `"launch_fooocus_installer.ps1`" -ForegroundColor Yellow
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main fooocus update `$launch_args
...
function Get-InstallerCmdletHelp {
    `$content = `"
使用:
    .\`$(`$script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-BuildMode] [-BuildWitchBranch <Fooocus 分支编号>] [-DisableUpdate] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像源地址>]
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main fooocus switch `$launch_args
...
function Main {
    ...
    if (`$status) {
        ...
        try {
            Invoke-Expression `"& ```"(Join-NormalizedPath `$PSScriptRoot 'cache' 'fooocus_installer.ps1')```" `$extra_args @arg`"
        }
        catch {
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main fooocus reinstall-pytorch `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    & python -m sd_webui_all_in_one.cli_manager.main fooocus model install-library `$launch_args
...
function Main {
    ...
    if (Test-Path (Join-NormalizedPath `$env:FOOOCUS_INSTALLER_ROOT `$env:CORE_PREFIX `"python`" `"python.exe`")) {
        `$env:UV_PYTHON = Join-NormalizedPath `$env:FOOOCUS_INSTALLER_ROOT `$env:CORE_PREFIX `"python`" `"python.exe`"
    }
...
function Get-InstallerCmdletHelp {
    $content = "
使用:
    .\$($script:MyInvocation.MyCommand.Name) [-Help] [-CorePrefix <内核路径前缀>] [-InstallPath <安装 Fooocus 的绝对路径>] [-PyTorchMirrorType <PyTorch 镜像源类型>] [-UseUpdateMode] [-DisablePyPIMirror] [-DisableProxy] [-UseCustomProxy <代理服务器地址>] [-DisableUV] [-DisableGithubMirror] [-UseCustomGithubMirror <Github 镜像站地址>] [-BuildMode] [-BuildWithUpdate] [-BuildWithLaunch] [-BuildWithTorch <PyTorch 版本编号>] [-BuildWithTorchReinstall] [-BuildWitchModel <模型编号列表>] [-BuildWitchBranch <Fooocus 分支编号>] [-NoPreDownloadNode] [-NoPreDownloadModel] [-PyTorchPackage <PyTorch 软件包>] [-InstallHanamizuki] [-NoCleanCache] [-xFormersPackage <xFormers 软件包>] [-DisableUpdate] [-DisableHuggingFaceMirror] [-UseCustomHuggingFaceMirror <HuggingFace 镜像源地址>] [-LaunchArg <Fooocus 启动参数>] [-EnableShortcut] [-DisableCUDAMalloc] [-DisableEnvCheck]
