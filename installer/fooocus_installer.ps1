...
"@)][string]$UseCustomGithubMirror,
    ...
    [Parameter(HelpMessage=@"
指定 Fooocus Installer 安装的 Fooocus 分支 (fooocus_main, ruined_fooocus_main, fooocus_mre_main)
未指定该参数时, 默认安装 lllyasviel - Fooocus 分支
支持指定安装的分支如下:
    fooocus_main:           lllyasviel - Fooocus 分支
    ruined_fooocus_main:    runew0lf - RuinedFooocus 分支
    fooocus_mre_main:       MoonRide303 - Fooocus-MRE 分支
...
"@)][string]$LaunchArg,
    ...
    [Parameter(HelpMessage=@"
(仅在 Fooocus Installer 构建模式下生效, 并且只作用于 Fooocus Installer 管理脚本) 禁用 Hotpatcher 补丁系统注入
...
"@)][switch]$DisableCUDAMalloc,
    ...
    [Parameter(HelpMessage=@"
(仅在 Fooocus Installer 构建模式下生效, 且只作用于 Fooocus Installer 管理脚本) 禁用 Fooocus Installer 检查 Fooocus 运行环境中存在的问题, 禁用后可能会导致 Fooocus 环境中存在的问题无法被发现并修复
...
$script:FOOOCUS_INSTALLER_VERSION = 423
...
# 环境变量
$env:FOOOCUS_PATH = Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX
$env:FOOOCUS_ROOT = Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX
...
$env:SD_WEBUI_ALL_IN_ONE_LOGGER_NAME = "Fooocus Installer"
...
function Get-Version {
    $ver = $([string]$script:FOOOCUS_INSTALLER_VERSION).ToCharArray()
    $major = ($ver[0..($ver.Length - 3)])
    $minor = $ver[-2]
    $micro = $ver[-1]
    Write-Log "Fooocus Installer 版本: v${major}.${minor}.${micro}"
}
...
function Get-InstallBranch {
    $branch_mapping_table = @(
        @{ Key = "fooocus";             Val = "fooocus_main" }
        @{ Key = "fooocus_main";        Val = "fooocus_main" }
        @{ Key = "ruined_fooocus";      Val = "ruined_fooocus_main" }
        @{ Key = "ruined_fooocus_main"; Val = "ruined_fooocus_main" }
        @{ Key = "fooocus_mre";         Val = "fooocus_mre_main" }
        @{ Key = "fooocus_mre_main";    Val = "fooocus_mre_main" }
    )
    $target_branch = $null
    foreach ($item in $branch_mapping_table) {
        $file_path = Join-NormalizedPath $PSScriptRoot "install_$($item.Key).txt"
    ...
    foreach ($runtime_name in $runtime_names) {
        ...
        if (!(Test-Path -LiteralPath $source_path -PathType Container)) {
            if (($runtime_name -eq "python") -or ((Get-CurrentPlatform) -eq "windows")) {
                Write-Log "环境缺少 $display_name, 无法为 $LauncherName 准备 $display_name 环境, 请重新运行 Fooocus Installer 修复环境" -Level WARNING
                $runtime_ready = $false
            }
            continue
        }
...
function Test-InstallerGitKernelSnapshot {
    ...
    if ($null -eq $Snapshot.kernel) {
        Stop-SnapshotRestore "快照缺少内核信息, 无法在 Installer 重建模式中恢复 Fooocus"
    }
    if (-not [System.Convert]::ToBoolean($Snapshot.kernel.is_git_repo)) {
        Stop-SnapshotRestore "快照中的 Fooocus 内核不是 Git 仓库, Installer 重建模式无法恢复该内核"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.url)) {
        Stop-SnapshotRestore "快照中的 Fooocus 内核缺少 Git 远程地址, 无法从头重建环境"
    }
    if ([string]::IsNullOrWhiteSpace([string]$Snapshot.kernel.commit)) {
        Stop-SnapshotRestore "快照中的 Fooocus 内核缺少 Git commit, 无法恢复到快照版本"
    }
}
...
function Resolve-SnapshotRebuildConfig {
    ...
    if ($snapshot_webui_type -ne "fooocus") {
        Stop-SnapshotRestore "快照 WebUI 类型不匹配: 期望 'fooocus', 实际 '$snapshot_webui_type'"
    }
...
function Invoke-Installation {
    ...
    if ($script:RestoreFromSnapshot) {
        ...
        $snapshot_path = Get-NormalizedFilePath $script:SnapshotPath
        $core_cli_command = @("python", "-m", "sd_webui_all_in_one", "fooocus", "restore", $snapshot_path)
        & python -m sd_webui_all_in_one fooocus restore $snapshot_path $launch_params
        $operation_label = "恢复"
    } else {
    ...
    $target_branch = Get-InstallBranch
    $launch_args_map = @{
        "fooocus_main"          = "--language zh --preset sd_webui_all_in_one --disable-offload-from-vram --disable-analytics --always-download-new-model"
        "ruined_fooocus_main"   = "--language zh --preset sd_webui_all_in_one --disable-offload-from-vram --disable-analytics --always-download-new-model"
        "fooocus_mre_main"      = "--language zh --preset sd_webui_all_in_one --disable-offload-from-vram --disable-analytics --always-download-new-model"
    }
    if (!(Test-Path (Join-NormalizedPath $script:InstallPath "launch_args.txt"))) {
        Write-Log "设置默认 Fooocus 启动参数"
        if (($target_branch)-and $launch_args_map.ContainsKey($target_branch)) {
            $default_content = $launch_args_map[$target_branch]
        } else {
            $default_content = $launch_args_map["fooocus_main"]
        }
...
function Initialize-EnvPath {
    ...
    `$env:FOOOCUS_PATH = Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX
    `$env:FOOOCUS_ROOT = Join-NormalizedPath `$PSScriptRoot `$env:CORE_PREFIX
...
function Update-Installer {
    ...
    if (`$script:BuildMode) {
        Write-Log `"Fooocus Installer 构建模式已启用, 跳过 Fooocus Installer 更新检查`"
        return
    }
    ...
    foreach (`$url in `$urls) {
        Write-Log `"检查 Fooocus Installer 更新中`"
        try {
            `$web_request_params = @{
                Uri = `$url
                UseBasicParsing = `$true
                OutFile = (Join-NormalizedPath `$env:CACHE_HOME `"fooocus_installer.ps1`")
                TimeoutSec = 15
                ErrorAction = `"Stop`"
            }
            Invoke-WebRequest @web_request_params
            `$latest_version = [int]`$(
                Get-Content (Join-NormalizedPath `$env:CACHE_HOME `"fooocus_installer.ps1`") -Encoding UTF8 |
            ...
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试检查 Fooocus Installer 更新中`" -Level WARNING
            } else {
                Write-Log `"检查 Fooocus Installer 更新失败`" -Level ERROR
                return
            }
        }
    }

    if (`$latest_version -le `$script:FOOOCUS_INSTALLER_VERSION) {
        Write-Log `"Fooocus Installer 已是最新版本`"
        return
    }

    Write-Log `"调用 Fooocus Installer 进行更新中`"
    & (Join-NormalizedPath `$env:CACHE_HOME `"fooocus_installer.ps1`") -InstallPath `"`$PSScriptRoot`" -UseUpdateMode
    ...
    Write-Log `"更新结束, 重新启动 Fooocus Installer 管理脚本中, 使用的命令行参数: `$raw_params`"
...
function Test-PythonAndGit {
    ...
    if (`$python_cmd) {
        ...
        if (-not ((`$python_cmd) -and ((`$python_cmd.ToString().StartsWith(`$python_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$python_cmd.ToString().StartsWith(`$python_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
            Write-Log `"检测到当前使用的 Python 路径为 `${python_cmd}, 但未在 `${python_path_prefix} 或 `${python_extra_path_prefix} 这两个受 Fooocus Installer 管理的 Python 路径, 即当前正在使用外部的 Python 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_fooocus_installer.ps1 修复运行环境`" -Level ERROR
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Python, 这将导致运行时发生异常, 请运行 launch_fooocus_installer.ps1 修复运行环境`" -Level ERROR
    }
    ...
    if (`$git_cmd) {
        if ((Get-CurrentPlatform) -eq `"windows`") {
            ...
            if (-not ((`$git_cmd) -and ((`$git_cmd.ToString().StartsWith(`$git_path_prefix, [System.StringComparison]::OrdinalIgnoreCase)) -or (`$git_cmd.ToString().StartsWith(`$git_extra_path_prefix, [System.StringComparison]::OrdinalIgnoreCase))))) {
                Write-Log `"检测到当前使用的 Git 路径为 `${git_cmd}, 但未在 `${git_path_prefix} 或 `${git_extra_path_prefix} 这两个受 Fooocus Installer 管理的 Git 路径, 即当前正在使用外部的 Git 环境, 这可能会导致一些运行环境问题, 可尝试运行 launch_fooocus_installer.ps1 修复运行环境`" -Level ERROR
            }
        }
    } else {
        Write-Log `"检测到当前环境中未安装任何 Git, 这将导致运行时发生异常, 请运行 launch_fooocus_installer.ps1 修复运行环境`" -Level ERROR
    }
}
...
`"@)][switch]`$DisableUV,
    ...
    [Parameter(HelpMessage=@`"
设置 Fooocus 自定义启动参数, 如启用 --fast 和 --auto-launch, 则使用 -LaunchArg ```"--fast --auto-launch```" 进行启用
`"@)][string]`$LaunchArg,
...
function Main {
    ...
    if (`$script:BuildMode) {
        ...
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"check-env`")
        & python -m sd_webui_all_in_one fooocus check-env `$launch_args
        `$exit_code = Get-NativeCommandExitCode -Success `$?
        if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
    } else {
        `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"launch`")
        & python -m sd_webui_all_in_one fooocus launch `$launch_args
        `$exit_code = Get-NativeCommandExitCode -Success `$?
        if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
        if (`$exit_code -eq 0) {
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"update`")
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"switch`")
    Set-SnapshotCliArgs `$launch_args
    & python -m sd_webui_all_in_one fooocus switch `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }
...
function Download-Installer {
    ...
    foreach (`$url in `$urls) {
        Write-Log `"正在下载最新的 Fooocus Installer 脚本`"
        `$web_request_params = @{
            Uri = `$url
            UseBasicParsing = `$true
            OutFile = (Join-NormalizedPath `$PSScriptRoot `"cache`" `"fooocus_installer.ps1`")
            TimeoutSec = 15
            ErrorAction = `"Stop`"
        }
        try {
            Invoke-WebRequest @web_request_params
            Write-Log `"下载 Fooocus Installer 脚本成功`"
            break
        } catch {
            Write-Log `"下载 Fooocus Installer 脚本失败: `$(`$_.Exception.Message)`" -Level ERROR
            `$i += 1
            if (`$i -lt `$urls.Length) {
                Write-Log `"重试下载 Fooocus Installer 脚本`" -Level WARNING
            } else {
                Write-Log `"下载 Fooocus Installer 脚本失败, 可尝试重新运行 Fooocus Installer 下载脚本`" -Level ERROR
                return `$false
            }
        }
    }
...
function Main {
    ...
    if (`$status) {
        Write-Log `"运行 Fooocus Installer 中`"
        `$arg = Get-LocalSetting
        `$extra_args = Get-ExtraArgs
        `$script_path = Join-NormalizedPath `$PSScriptRoot `"cache`" `"fooocus_installer.ps1`"
        try {
            Invoke-Expression `"& ```"`$script_path```" `$extra_args @arg`" -ErrorAction Stop
        }
        catch {
            Write-Log `"运行 Fooocus Installer 时出现了错误: `$_`" -Level ERROR
...
".Trim()
    ...
    Write-Log "$(if (Test-Path (Join-NormalizedPath $script:InstallPath "launch_fooocus_installer.ps1")) { "更新" } else { "生成" }) launch_fooocus_installer.ps1 中"
    Write-FileWithStreamWriter -Encoding UTF8BOM -Path (Join-NormalizedPath $script:InstallPath "launch_fooocus_installer.ps1") -Value $content
}
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"reinstall-pytorch`")
    Set-SnapshotCliArgs `$launch_args
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"model`", `"install-library`")
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"gui`", `"version-manager`")
    Set-SnapshotCliArgs `$launch_args
    & python -m sd_webui_all_in_one fooocus gui version-manager `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }

    Write-Log `"退出 Fooocus 版本管理脚本`"
...
function Main {
    ...
    `$launch_args = Get-LaunchCoreArgs
    `$core_cli_command = @(`"python`", `"-m`", `"sd_webui_all_in_one`", `"fooocus`", `"gui`", `"snapshot-manager`")
    & python -m sd_webui_all_in_one fooocus gui snapshot-manager `$launch_args
    `$exit_code = Get-NativeCommandExitCode -Success `$?
    if (`$exit_code -ne 0) { Write-CoreCliFailureCommand -CommandPrefix `$core_cli_command -Arguments `$launch_args -ExitCode `$exit_code }

    Write-Log `"退出 Fooocus 快照管理脚本`"
...
function Test-EnvIntegrity {
    `$items = @(
        ...
        @{ n=`"Fooocus`"; p=(Join-NormalizedPath `$env:CORE_PREFIX `"main.py`"); t=`"file`" },
        @{ n=`"uv`"; m=`"uv`" },
        @{ n=`"PyTorch`"; m=`"torch`" },
...
function Main {
    ...
    while (`$true) {
        ...
        switch (`$choice) {
            ...
            `"19`" { Start-Process `"https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/`" }
...
if exist .\hanamizuki.exe (
    ...
    Move-PortableLauncherRuntime -InstallRoot `$env:FOOOCUS_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"绘世启动器`" | Out-Null
}


# 安装 Hanafubuki 启动器
function global:Install-Hanafubuki {
    `$core_path = Join-NormalizedPath `$env:FOOOCUS_INSTALLER_ROOT `$env:CORE_PREFIX
    if (!(Test-Path -LiteralPath `$core_path -PathType Container)) {
        Write-Log `"内核路径 `$core_path 未找到, 无法安装 Hanafubuki 启动器, 请检查 Fooocus 是否已正确安装, 或者尝试运行 Fooocus Installer 进行修复`" -Level ERROR
        return
    }

    `$installed = Install-HanafubukiPackage -InstallRoot `$env:FOOOCUS_INSTALLER_ROOT -CachePath `$env:CACHE_HOME
    if (`$installed) {
        Move-PortableLauncherRuntime -InstallRoot `$env:FOOOCUS_INSTALLER_ROOT -CorePrefix `$env:CORE_PREFIX -LauncherName `"Hanafubuki 启动器`" | Out-Null
    }
}
...
这是 Fooocus 的安装后速查说明。完整安装、启动、模型、更新和故障排查说明请阅读文档站。
Fooocus Installer 文档：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/

一、安装完成后先做什么
1. 运行 launch.ps1 启动 Fooocus。启动成功后，浏览器会打开或终端会显示访问地址。
2. 需要模型时运行 download_models.ps1；模型放置位置和推荐资源见模型与资源文档。
3. 需要更新 Fooocus 时运行 update.ps1。
4. 需要切换 Fooocus 分支时运行 switch_branch.ps1。
5. 需要执行 Python、Pip、Git 命令时运行 terminal.ps1，或先运行 activate.ps1 激活环境。

二、常用脚本速查
- launch.ps1：启动 Fooocus。
- update.ps1：更新 Fooocus 和管理脚本。
- download_models.ps1：下载模型到 Fooocus 的模型目录。
...
- 启动、安装或更新报错：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/troubleshooting/
- 不确定脚本参数：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/commands/
- 下载器或 Launcher 出错：https://licyk.github.io/sd-webui-all-in-one/tools/troubleshooting/

五、详细文档
- 启动与使用：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/usage/
- 模型与资源：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/resources/
- 配置与镜像：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/config/
- 维护与迁移：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/maintenance/
- 常用命令：https://licyk.github.io/sd-webui-all-in-one/installer/fooocus/commands/
...
function Use-InstallMode {
    ...
    if ($script:BuildMode) {
        ...
        Write-Log "Fooocus 环境构建完成, 路径: $script:InstallPath"
    } else {
        Install-Hanamizuki
        Install-Hanafubuki | Out-Null
        Initialize-HanamizukiEnv
        Initialize-HanafubukiEnv
        Write-Log "Fooocus 安装结束, 安装路径为: $script:InstallPath"
    }

    Save-InstallResultSnapshot -CliName "fooocus" -WebUIPath (Join-NormalizedPath $script:InstallPath $env:CORE_PREFIX)
...
function Main {
    ...
    if ($script:UseUpdateMode) {
        ...
        if ($script:BuildMode) { Write-Log "Fooocus Installer 构建模式已启用" }
        Write-Log "使用安装模式"
        Use-InstallMode
    }
}
