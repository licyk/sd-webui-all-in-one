...
|[下载地址 3](https://github.com/licyk/sd-webui-all-in-one/raw/main/installer/comfyui_installer.ps1)|
|[下载地址 4](https://gitee.com/licyk/sd-webui-all-in-one/raw/main/installer/comfyui_installer.ps1)|
|[下载地址 5](https://gitlab.com/licyk/sd-webui-all-in-one/-/raw/main/installer/comfyui_installer.ps1)|
...
>[!NOTE]  
>实际路径需要根据内核路径前缀来决定，如果内核路径前缀为`core`，则实际移动到的路径分别为`ComfyUI/core/python`和`ComfyUI/core/git`。建议通过自动安装绘世启动器的方法来安装绘世启动器。
...
>[!NOTE]  
>如果不修改名称，需要根据[设置内核路径前缀](#设置内核路径前缀)中的说明配置内核路径前缀。在这个例子中内核路径前缀就需要设置为`ComfyUI-aki-v1.2`。
...
ComfyUI Installer 通过路径前缀在安装目录中寻找 ComfyUI 内核并使用。查找时通过遍历 ComfyUI Installer 内部预设的列表，若该预设名有对应的文件夹名，则将该预设名作为内核路径前缀，并对该文件夹中的内核进行启动和管理。当未找到任何内核文件夹时，使用默认的内核路径前缀`core`。

内核路径前缀可手动指定，若内核文件夹在脚本所在路径中的名称为`ComfyUI-aki-v1`，此时可在当前路径创建`core_prefix.txt`文件，并在文件中将刚刚的名称写进该文件中，即`ComfyUI-aki-v1`，再保存文件，此时 ComfyUI Installer 将对该内核文件夹进行启动和管理。

内核路径前缀除了可以使用名称，还可以使用绝对路径或者相对路径，即 ComfyUI Installer 可以启动和管理在当前脚本所在路径之外的 ComfyUI。比如 ComfyUI 所在路径为`D:/Tools/AI/ComfyUI-aki-v1.1`。如果使用绝对路径，则直接将这个路径作为内核路径前缀，推荐使用这个方式，比较简单。

如果使用相对路径，此时需要知道 ComfyUI Installer 所在路径，比如`D:/Downloads/ComfyUI`，则可以得出内核路径前缀为`../../Tools/AI/ComfyUI-aki-v1.1`。

相对路径可使用[命令的使用](#命令的使用)中的[计算 ComfyUI 内核路径前缀](#计算-comfyui-内核路径前缀)进行计算，或者直接使用`settings.ps1`配置内核路径前缀，可自动将 ComfyUI 内核的绝对路径转换为内核路径前缀。
...
|参数|作用|
|---|---|
|`-Help`|获取 ComfyUI Installer 的帮助信息。|
|`-CorePrefix` <内核路径前缀>|设置内核的路径前缀，默认路径前缀为 `core`。|
|`-InstallPath` <安装 ComfyUI 的绝对路径>|指定 ComfyUI Installer 安装 ComfyUI 的路径，使用绝对路径表示。<br>例如：`./comfyui_installer.ps1 -InstallPath "D:\Download"`，这将指定安装到 D:\Download 路径。|
...
|`-UseUpdateMode`|指定 ComfyUI Installer 使用更新模式，只对 ComfyUI Installer 的管理脚本进行更新。|
...
|`-DisableUV`|禁用 ComfyUI Installer 使用 uv 安装 Python 软件包，改用 Pip 安装。|
...
|`-BuildMode`|启用构建模式，在基础安装结束后将调用管理脚本执行剩余任务。出现错误时不再暂停而是直接退出。<br>多个脚本将按以下优先级执行：<br><li>`reinstall_pytorch.ps1`：对应`-BuildWithTorch`/`-BuildWithTorchReinstall`<br><li>`download_models.ps1`：对应`-BuildWithModel`<br><li>`update.ps1`：对应`-BuildWithUpdate`<br><li>`update_node.ps1`：对应`-BuildWithUpdateNode`<br><li>`launch.ps1`：对应`-BuildWithLaunch`|
...
|`-BuildWithUpdate`|(需添加`-BuildMode`) 安装流程结束后调用 `update.ps1` 脚本，更新 ComfyUI 内核。|
|`-BuildWithUpdateNode`|(需添加`-BuildMode`) 安装流程结束后调用 `update_node.ps1` 脚本，更新 ComfyUI 扩展。|
|`-BuildWithLaunch`|(需添加`-BuildMode`) 安装流程结束后调用 `launch.ps1` 脚本，执行启动前的环境检查，但跳过启动 ComfyUI。|
...
|`-DisableUpdate`|(仅在构建模式生效且只作用于管理脚本) 禁用 ComfyUI Installer 更新检查。|
...
|`-LaunchArg` <ComfyUI 启动参数>|(仅在构建模式生效且只作用于管理脚本) 设置 ComfyUI 自定义启动参数。如：`-LaunchArg "--fast --auto-launch"`|
|`-EnableShortcut`|(仅在构建模式生效且只作用于管理脚本) 创建 ComfyUI 启动快捷方式。|
