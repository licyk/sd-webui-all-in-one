...
![GUI 一键启动安装模式](../assets/images/tools/launcher-gui/one-click-install-mode.jpg)

未安装或安装不完整时，使用“安装模式”借助安装器完成安装。安装前建议先到“高级选项”确认安装路径、安装分支、镜像和代理。

![GUI 一键启动启动模式](../assets/images/tools/launcher-gui/one-click-launch-mode.jpg)
...
- `model_manager.ps1`：打开模型管理器，管理模型文件或 InvokeAI 模型记录。
...
![GUI 安装路径设置](../assets/images/tools/launcher-gui/advanced-install-path.jpg)
...
![GUI 安装器设置](../assets/images/tools/launcher-gui/advanced-installer-settings.jpg)

“安装器设置”只显示当前项目支持的安装参数，例如核心路径前缀、PyTorch 镜像类型、Python 版本、自定义代理、GitHub 镜像、HuggingFace 镜像和预下载开关。这些参数用于全新安装或重新运行安装器修复环境。

![GUI 管理脚本设置](../assets/images/tools/launcher-gui/advanced-script-settings.jpg)

“管理脚本设置”用于给 `launch.ps1`、`download_models.ps1`、`model_manager.ps1`、`reinstall_pytorch.ps1`、`settings.ps1`、`switch_branch.ps1`、`version_manager.ps1` 等脚本保存默认参数。额外原始参数会追加到结构化参数之后。

## 模型管理

模型管理器可通过管理脚本或 CLI 的 `gui model-manager` 子命令启动。Stable Diffusion WebUI、ComfyUI、Fooocus 会管理安装目录下的 `models/`，SD Trainer 和 SD Trainer Script 会管理 `sd-models/`。界面支持新建文件夹、复制、移动、删除、导入本地模型文件或文件夹，以及输入 URL 下载到选定目录。

InvokeAI 使用自身模型管理器，不把模型目录当作普通文件夹处理。它支持刷新模型记录、从 URL 安装、复制导入本地模型，以及按 InvokeAI 默认规则删除/注销模型；本地导入会先复制源文件，原文件不会被移动。删除/注销时，位于 InvokeAI 管理目录内的模型文件会被同步删除，就地安装的外部模型文件只会保留并注销记录。

## 软件选择

![GUI 软件选择](../assets/images/tools/launcher-gui/software-selection.jpg)

在“软件选择”页面切换要安装或管理的 WebUI / 工具。切换后会自动刷新安装状态和可用脚本。

## 启动器设置

![GUI 启动器设置](../assets/images/tools/launcher-gui/settings.jpg)
...
![GUI 关于页面](../assets/images/tools/launcher-gui/about.jpg)
