...
默认配置文件固定为 `init.ps1` 同级目录的 `patcher_config.json`。Hotpatcher 默认启用且未指定 `-HotpatcherConfig` 时，如果该文件不存在，`init.ps1` 会自动导出默认配置；如果指定了 `-HotpatcherConfig`，脚本只使用指定路径，不会自动创建配置文件。
