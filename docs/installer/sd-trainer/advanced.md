...
### 使用 Hanafubuki 启动器
可在安装 SD Trainer 时添加 `-InstallHanafubuki`，或进入 SD Trainer Env 后运行 `Install-Hanafubuki`。安装器会按 Hugging Face、ModelScope 的顺序读取版本清单，并下载当前平台和架构对应的安装包。
...
- `-DisableEnvCheck`：(仅在构建模式生效且只作用于管理脚本) 禁用检查 SD Trainer 运行环境问题。
...
如果要安装 [wochenlong/SD Trainer Next](https://github.com/wochenlong/lora-scripts-next)，则将 `-InstallBranch` 设置为 `sd_trainer_next_main`。

```powershell
./sd_trainer_installer.ps1 -InstallPath "D:/Download" -InstallBranch "sd_trainer_next_main"
```
...
构建模式需要使用命令行参数进行启用，具体可阅读 [使用命令运行 SD Trainer Installer](advanced.md#sd-trainer-installer_1) 中的参数说明。
