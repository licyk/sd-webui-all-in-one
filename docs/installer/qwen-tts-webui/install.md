# Qwen TTS WebUI Installer 环境准备与安装

## 快速流程

1. Windows 用户先在“环境配置脚本下载”中下载环境配置脚本 `configure_env.bat` 并运行；Linux / macOS 用户先安装 PowerShell，macOS 还需要安装 Homebrew。
2. 在“Qwen TTS WebUI Installer 下载地址”中下载 Qwen TTS WebUI Installer 安装脚本 `qwen_tts_webui_installer.ps1`。
3. 将 Qwen TTS WebUI Installer 安装脚本 `qwen_tts_webui_installer.ps1` 放到希望安装 Qwen TTS WebUI 的位置，按需创建镜像或代理配置文件。
4. 右键 Qwen TTS WebUI Installer 安装脚本 `qwen_tts_webui_installer.ps1` 选择 `使用 PowerShell 运行`，不要左键双击 `.ps1` 脚本；或在终端中使用 `pwsh qwen_tts_webui_installer.ps1`。
...
[GitHub Release 下载 :material-download:](https://github.com/licyk/sd-webui-all-in-one/releases/download/qwen_tts_webui_installer/qwen_tts_webui_installer.ps1){ .md-button .md-button--primary }
[Gitee Release 下载 :material-download:](https://gitee.com/licyk/sd-webui-all-in-one/releases/download/qwen_tts_webui_installer/qwen_tts_webui_installer.ps1){ .md-button }
[GitHub Raw 下载 :material-download:](https://github.com/licyk/sd-webui-all-in-one/raw/main/installer/qwen_tts_webui_installer.ps1){ .md-button }
[Gitee Raw 下载 :material-download:](https://gitee.com/licyk/sd-webui-all-in-one/raw/main/installer/qwen_tts_webui_installer.ps1){ .md-button }
[GitLab Raw 下载 :material-download:](https://gitlab.com/licyk/sd-webui-all-in-one/-/raw/main/installer/qwen_tts_webui_installer.ps1){ .md-button }

将 Qwen TTS WebUI Installer 下载至本地，右键`qwen_tts_webui_installer.ps1`脚本，在弹出的右键菜单中点击`使用 PowerShell 运行`，此时 Qwen TTS WebUI Installer 将安装 Qwen TTS WebUI 至本地。
...
!!! note
    ...
    2. Qwen TTS WebUI Installer 支持使用在命令行中通过参数配置 Qwen TTS WebUI 的安装参数，具体说明可阅读[使用命令运行 Qwen TTS WebUI Installer](advanced.md#qwen-tts-webui-installer_1)。
