...
!!! note
    实际路径会根据当前内核路径前缀决定：Python 会移动到 `<安装目录>/<内核路径前缀>/python`，Git 会移动到 `<安装目录>/<内核路径前缀>/git`。例如安装目录为 `Fooocus` 且内核路径前缀为 `core` 时，实际路径分别为 `Fooocus/core/python` 和 `Fooocus/core/git`。建议通过自动安装绘世启动器的方法来安装绘世启动器。
...
再下载绘世启动器放到 `<安装目录>/<内核路径前缀>` 目录中，就可以通过启动器启动 Fooocus。
...
在脚本同级目录创建 `enable_shortcut.txt` 文件，当运行 `launch.ps1` 时将会自动创建快捷启动方式，下次启动时可以使用快捷方式启动 Fooocus。
...
- `-InstallPath` `<安装 Fooocus 的绝对路径>`：指定 Fooocus Installer 安装 Fooocus 的路径，使用绝对路径表示。
    例如：`./fooocus_installer.ps1 -InstallPath "D:\Download"`，这将指定安装到 D:\Download 路径。
...
- `-InstallBranch` `<安装的 Fooocus 分支>`：指定安装的 Fooocus 分支。未指定时默认安装 `fooocus_main`。
    支持的分支如下：
...
- `-DisableEnvCheck`：(仅在构建模式生效且只作用于管理脚本) 禁用检查 Fooocus 运行环境问题。
...
构建模式需要使用命令行参数进行启用，具体可阅读 [使用命令运行 Fooocus Installer](advanced.md#fooocus-installer_1) 中的参数说明。
