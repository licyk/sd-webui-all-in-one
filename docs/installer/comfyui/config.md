...
### 设置 HuggingFace 镜像
!!! info
    该设置可通过 [管理 ComfyUI Installer 设置](#管理-comfyui-installer-设置) 中提到的 `settings.ps1` 进行修改。
...
### 设置内核路径前缀
!!! info
    该设置可通过 [管理 ComfyUI Installer 设置](#管理-comfyui-installer-设置) 中提到的 `settings.ps1` 进行修改，也可以通过 `core_prefix.txt` 或 `-CorePrefix` 参数指定。
