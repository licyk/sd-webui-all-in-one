...
!!! note
    实际路径会根据当前内核路径前缀决定：Python 会移动到 `<安装目录>/<内核路径前缀>/python`，Git 会移动到 `<安装目录>/<内核路径前缀>/git`。例如安装目录为 `ComfyUI` 且内核路径前缀为 `core` 时，实际路径分别为 `ComfyUI/core/python` 和 `ComfyUI/core/git`。建议通过自动安装绘世启动器的方法来安装绘世启动器。
...
再下载绘世启动器放到 `<安装目录>/<内核路径前缀>` 目录中，就可以通过启动器启动 ComfyUI。
...
### 使用 Hanafubuki 启动器
可在安装 ComfyUI 时添加 `-InstallHanafubuki`，或进入 ComfyUI Env 后运行 `Install-Hanafubuki`。安装器会按 Hugging Face、ModelScope 的顺序读取版本清单，并下载当前平台和架构对应的安装包。
...
在脚本同级目录创建 `enable_shortcut.txt` 文件，当运行 `launch.ps1` 时将会自动创建快捷启动方式，下次启动时可以使用快捷方式启动 ComfyUI。
...
- `-BuildMode`：启用构建模式，在基础安装结束后将调用管理脚本执行剩余任务。出现错误时不再暂停而是直接退出。
    ...
    - `update_node.ps1`：对应`-BuildWithUpdateNode`
...
- `-DisableEnvCheck`：(仅在构建模式生效且只作用于管理脚本) 禁用检查 ComfyUI 运行环境问题。
...
构建模式需要使用命令行参数进行启用，具体可阅读 [使用命令运行 ComfyUI Installer](advanced.md#comfyui-installer_1) 中的参数说明。
