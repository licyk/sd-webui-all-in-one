...
- [使用](#使用)
  - [基础操作](#基础操作)
    ...
    - [设置 Qwen TTS WebUI 启动参数](#设置-qwen-tts-webui-启动参数)
  - [环境管理](#环境管理)
    - [进入 Qwen TTS WebUI 所在的 Python 环境](#进入-qwen-tts-webui-所在的-python-环境)
  ...
  - [维护与修复](#维护与修复)
    ...
    - [重装 Qwen TTS WebUI](#重装-qwen-tts-webui)
    - [重装 Python 环境](#重装-python-环境)
    - [重装 Git](#重装-git)
    - [重装 PyTorch](#重装-pytorch)
    - [卸载 Qwen TTS WebUI](#卸载-qwen-tts-webui)
    - [移动 Qwen TTS WebUI 的路径](#移动-qwen-tts-webui-的路径)
  - [高级功能](#高级功能)
    - [设置 uv 包管理器](#设置-uv-包管理器)
    - [创建快捷启动方式](#创建快捷启动方式)
    - [设置内核路径前缀](#设置内核路径前缀)
    - [管理 Qwen TTS WebUI Installer 设置](#管理-qwen-tts-webui-installer-设置)
    - [Qwen TTS WebUI Installer 对 Python / Git 环境的识别](#qwen-tts-webui-installer-对-python--git-环境的识别)
    - [使用命令运行 Qwen TTS WebUI Installer](#使用命令运行-qwen-tts-webui-installer)
    - [Qwen TTS WebUI Installer 构建模式和普通安装模式](#qwen-tts-webui-installer-构建模式和普通安装模式)
  - [更新管理](#更新管理)
    - [获取最新的 Qwen TTS WebUI Installer 并运行](#获取最新的-qwen-tts-webui-installer-并运行)
    - [更新 Qwen TTS WebUI 管理脚本](#更新-qwen-tts-webui-管理脚本)
      - [1. 直接更新](#1-直接更新)
  ...
  - [常用命令](#常用命令)
    ...
    - [查看并切换 Qwen TTS WebUI 的版本](#查看并切换-qwen-tts-webui-的版本-1)
...
>2. Qwen TTS WebUI Installer 支持使用在命令行中通过参数配置 Qwen TTS WebUI 的安装参数，具体说明可阅读[使用命令运行 Qwen TTS WebUI Installer](#使用命令运行-qwen-tts-webui-installer)。
...
### 查看并切换 Qwen TTS WebUI 的版本
```powershell
# 列出当前的所有版本
git -C Qwen TTS WebUI tag
...
git -C Qwen TTS WebUI reset --hard --recurse-submodules <Git Tag>

# 使用 git log 查看某个提交信息对应的 Hash 值
git -C Qwen TTS WebUI log
# 这将得到类似的输出信息
...
git -C Qwen TTS WebUI reset --hard --recurse-submodules <Git Commit Hash>
```
...
Qwen TTS WebUI Installer 生成的 PowerShell 脚本中已设置了 HuggingFace 镜像源，如果需要自定义 HuggingFace 镜像源，可以在和脚本同级的目录创建 `hf_mirror.txt` 文件，在文件中填写 HuggingFace 镜像源的地址后保存，再次启动脚本时将读取该文件的配置并设置 HuggingFace 镜像源。
...
Qwen TTS WebUI Installer 为了加速访问 Github 的速度，加快下载和更新 Qwen TTS WebUI 的速度，默认在启动脚本时自动检测可用的 Github 镜像源并设置。如果需要自定义 Github 镜像源，可以在和脚本同级的目录创建 `gh_mirror.txt` 文件，在文件中填写 Github 镜像源的地址后保存，再次启动脚本时将取消自动检测可用的 Github 镜像源，而是读取该文件的配置并设置 Github 镜像源。
...
Qwen TTS WebUI Installer 默认启用了 PyPI镜像源加速下载 Python 软件包，如果需要禁用 PyPI镜像源，可以在脚本同级目录创建 `disable_pypi_mirror.txt` 文件，再次运行脚本时将 PyPI 源切换至官方源。
...
将 Qwen TTS WebUI Installer 下载到本地后，在 Qwen TTS WebUI Installer 所在目录打开 PowerShell，使用命令运行，将 Qwen TTS WebUI Installer 的管理脚本安装到本地，比如在`D:/qwen-tts-webui`，则命令如下。
...
接下来需要将 Qwen TTS WebUI 移动到 `D:/qwen-tts-webui` 目录中，如果 Qwen TTS WebUI 的文件夹名称不是 `qwen-tts-webui`，比如`qwen_tts_webui_portable`，需要将名称修改成`qwen-tts-webui`。

>[!NOTE]  
>如果不修改名称，需要根据 [设置内核路径前缀](#设置内核路径前缀) 中的说明配置内核路径前缀。在这个例子中内核路径前缀就需要设置为 `qwen_tts_webui_portable`。
...
Qwen TTS WebUI Installer 默认使用了 uv 作为 Python 包管理器，大大加快管理 Python 软件包的速度（如安装 Python 软件包）。
...
Qwen TTS WebUI Installer 通过路径前缀在安装目录中寻找 Qwen TTS WebUI 内核并使用。查找时通过遍历 Qwen TTS WebUI Installer 内部预设的列表，若该预设名有对应的文件夹名，则将该预设名作为内核路径前缀，并对该文件夹中的内核进行启动和管理。当未找到任何内核文件夹时，使用默认的内核路径前缀`core`。

内核路径前缀可手动指定，若内核文件夹在脚本所在路径中的名称为 `Qwen-TTS-WebUI-aki-v1`，此时可在当前路径创建 `core_prefix.txt` 文件，并在文件中将刚刚的名称写进该文件中，即 `Qwen-TTS-WebUI-aki-v1`，再保存文件，此时 Qwen TTS WebUI Installer 将对该内核文件夹进行启动和管理。

内核路径前缀除了可以使用名称，还可以使用绝对路径或者相对路径，即 Qwen TTS WebUI Installer 可以启动和管理在当前脚本所在路径之外的 Qwen TTS WebUI。比如 Qwen TTS WebUI 所在路径为`D:/Tools/AI/Qwen-TTS-WebUI-aki-v1.1`。如果使用绝对路径，则直接将这个路径作为内核路径前缀，推荐使用这个方式，比较简单。
...
### Qwen TTS WebUI Installer 对 Python / Git 环境的识别
Qwen TTS WebUI Installer 通常情况下不会去调用系统环境中的 Python / Git，所以在安装过程会安装一个独立的 Python / Git 避免收到系统环境中的 Python / Git 影响。

Qwen TTS WebUI Installer 可以识别到的 Python 路径为 `qwen-tts-webui/python` 和`qwen-tts-webui/qwen-tts-webui/python`，当两者同时存在时，优先使用后者。

可以识别到的 Git 路径为 `qwen-tts-webui/git` 和`qwen-tts-webui/qwen-tts-webui/git`，当两者同时存在时，优先使用后者。

如果这两个路径 Python / Git 都不存在时，此时 Qwen TTS WebUI 的管理脚本将会调用系统环境中的 Python / Git，这可能会带来不好的结果，所以出现这种情况时就需要运行 Qwen TTS WebUI Installer 重新安装 Python / Git。
...
构建模式需要使用命令行参数进行启用，具体可阅读 [使用命令运行 Qwen TTS WebUI Installer](#使用命令运行-qwen-tts-webui-installer) 中的参数说明。
...
### 获取最新的 Qwen TTS WebUI Installer 并运行
运行 `launch_qwen_tts_webui_installer.ps1` 脚本，如果下载成功将会把 Qwen TTS WebUI Installer 下载到 `cache` 目录中并运行。
...
#### 1. 直接更新
当检测到有新版的 Qwen TTS WebUI Installer 时将自动应用更新。


#### 2. 使用 Qwen TTS WebUI Installer 配置管理器进行更新
运行 `settings.ps1`，选择 `更新 Qwen TTS WebUI Installer 管理脚本` 功能进行更新，更新完成后需关闭 Qwen TTS WebUI Installer 管理脚本以应用更新。


#### 3. 运行 Qwen TTS WebUI Installer 进行更新
...
```
.\qwen_tts_webui_installer.ps1 : 无法加载文件 D:\qwen-tts-webui\qwen_tts_webui_installer.ps1。
未对文件 D:\qwen-tts-webui\qwen_tts_webui_installer.ps1 进行数字签名。无法在当前系统上运行该脚本。
有关运行脚本和设置执行策略的详细信息，请参阅 https:/go.microsoft.com/fwlink/?LinkID=135170 中的 about_Execution_Policies。
所在位置 行:1 字符：1
+ .\qwen_tts_webui_installer.ps1
+ ~~~~~~~~~~~~~~~~~~~~~~~~
...
运行 Qwen TTS WebUI Installer 时出现以下类似的错误。
...
这是因为下载下来的 Python 软件包出现了损坏，Pip 无法进行安装，需要将 `qwen-tts-webui/cache/pip` 文件夹删除，再重新运行 Qwen TTS WebUI Installer。
...
启动 Qwen TTS WebUI 时出现以下的错误。
```
ERROR: [Error 13] error while attempting to bind on address ('127.0.0.1', 28000): 以一种访问权限不允许的方式做了一个访问套接字的尝试。
```

这是因为该端口被其他软件占用，Qwen TTS WebUI 无法使用。可尝试将占用该端口的软件关闭，或者在`launch.ps1` 所在目录创建`launch_args.txt` 文件，在该文件中写上启动参数把 Qwen TTS WebUI 端口修改，如`--port 8888`，保存 `launch_args.txt` 文件后使用 `launch.ps1` 重新启动 Qwen TTS WebUI。

>[!NOTE]  
>设置 Qwen TTS WebUI 启动参数的方法可参考 [设置 Qwen TTS WebUI 启动参数](#设置-Qwen-TTS-WebUI-启动参数)。
...
### 1. 使用自动环境激活脚本
运行 `terminal.ps1` 后将自动打开 PowerShell 并激活 Qwen TTS WebUI Env。
...
>[!IMPORTANT]  
>在 PowerShell 中一定要显示 `[Qwen TTS WebUI Env]` 才算进入了环境，这样才能使用下面的命令。
...
>[!NOTE]  
>推荐使用 `python -m pip` 的写法，`pip` 的写法也可用。Qwen TTS WebUI Installer 默认将 `pip` 命令链接到 `python -m pip` 避免直接调用 `pip`。  
