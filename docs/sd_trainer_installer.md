...
|`-DisableCUDAMalloc`|(仅在 SD-Trainer Installer 构建模式下生效，并且只作用于 SD-Trainer Installer 管理脚本) 禁用 SD-Trainer Installer 通过`PYTORCH_CUDA_ALLOC_CONF`/`PYTORCH_ALLOC_CONF`环境变量设置 CUDA 内存分配器。|
