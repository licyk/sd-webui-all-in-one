...
- [使用](#使用)
  ...
  - [更新管理](#更新管理)
    ...
    - [更新 SD-Trainer 管理脚本](#更新-sd-trainer-管理脚本)
      ...
      - [禁用 SD Trainer Installer 更新检查](#禁用-sd-trainer-installer-更新检查)
  - [故障排查](#故障排查)
  ...
  - [常用命令](#常用命令)
    ...
    - [查看并切换 SD-Trainer 的版本](#查看并切换-sd-trainer-的版本-1)
...
>[!NOTE]  
>如果不修改名称，需要根据 [设置内核路径前缀](#设置内核路径前缀) 中的说明配置内核路径前缀。在这个例子中内核路径前缀就需要设置为 `lora-scripts-v1.10.0`。
...
SD Trainer Installer 通过路径前缀在安装目录中寻找 SD-Trainer 内核并使用。查找时通过遍历 SD Trainer Installer 内部预设的列表，若该预设名有对应的文件夹名，则将该预设名作为内核路径前缀，并对该文件夹中的内核进行启动和管理。当未找到任何内核文件夹时，使用默认的内核路径前缀`core`。
...
|`-InstallPath` <安装 SD Trainer 的绝对路径>|指定 SD Trainer Installer 安装 SD Trainer 的路径，使用绝对路径表示。<br>例如：`./sd_trainer_installer.ps1 -InstallPath "D:\Download"`，这将指定安装到 D:\Download 路径。|
...
|`-InstallBranch` <安装的 SD Trainer 分支>|指定安装的 SD Trainer 分支。未指定时默认安装 Akegarasu/SD-Trainer。<br>支持的分支如下：<br><li>`sd_trainer_main`: Akegarasu - SD-Trainer 分支<br><li>`kohya_gui_main`: bmaltais - Kohya GUI 分支|
...
未对文件 D:\SD-Trainer\sd_trainer_installer.ps1 进行数字签名。无法在当前系统上运行该脚本。
