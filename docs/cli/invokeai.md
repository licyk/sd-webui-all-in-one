...
### 检查 InvokeAI 更新
```bash
sd-webui-all-in-one invokeai check-update [选项]
```

- **高级选项**:
  - `--invokeai-path <路径>`: InvokeAI 根目录。
  - `--no-pypi-mirror`: 不使用国内 PyPI 镜像源检查 InvokeAI 核心包版本。
  - `--no-github-mirror`: 不使用 Github 镜像源检查自定义节点。
...
#### 检查扩展更新
```bash
sd-webui-all-in-one invokeai custom-node check-update [选项]
```
