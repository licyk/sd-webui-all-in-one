...
### 检查 Fooocus 更新
```bash
sd-webui-all-in-one fooocus check-update [选项]
```
...
### 生成环境快照
```bash
sd-webui-all-in-one fooocus snapshot [选项]
```
...
### 恢复环境快照
```bash
sd-webui-all-in-one fooocus restore <快照文件> [选项]
```
...
### 快照管理 GUI
```bash
sd-webui-all-in-one fooocus gui snapshot-manager [选项]
```
