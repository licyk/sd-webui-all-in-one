...
### 检查 ComfyUI 更新
```bash
sd-webui-all-in-one comfyui check-update [选项]
```
...
### 生成环境快照
```bash
sd-webui-all-in-one comfyui snapshot [选项]
```
...
### 恢复环境快照
```bash
sd-webui-all-in-one comfyui restore <快照文件> [选项]
```
...
### 快照管理 GUI
```bash
sd-webui-all-in-one comfyui gui snapshot-manager [选项]
```
...
#### 检查扩展更新
```bash
sd-webui-all-in-one comfyui custom-node check-update [选项]
```
