...
整合包目录中还会出现一些中文命名的 `.bat` 文件，例如 `启动.bat`、`更新内核.bat`、`打开终端.bat`。这些脚本是构建整合包时由 `.github/make_docs.py` 生成的快捷入口，作用类似 alias：让没有阅读说明的 Windows 用户也能直接从中文文件名判断用途，并通过双击运行对应功能。
...
| `安装 SD WebUI All In One 启动器.bat` | 远程 Launcher 安装脚本 | 这是特殊脚本，不是本地 alias。它会从 GitHub / Gitee / GitLab 下载 SD WebUI All In One Launcher 的 `install.ps1`，再运行该安装脚本。 |
