...
- [Desktop Broker v2](docs/desktop-broker-protocol.md)：显式 desktop transport、HTTP 会话鉴权、事件/诊断确认与重放、active failure 和命令结果。
...
完整变量表和默认值见 [环境变量总览](docs/environment-variables.md)。
...
`sd_webui_all_in_one_hotpatcher.runtime` 默认使用原有 TCP + JSON Lines。每条消息是一行 UTF-8 JSON。宿主只需要开 TCP server，读写 JSONL 即可接入。只有显式设置 `SD_WEBUI_ALL_IN_ONE_HOTPATCHER_TRANSPORT_MODE=desktop_broker` 才使用独立的 Rust-owned HTTP broker；未设置、空值和显式 `legacy` 都继续使用 `RuntimeClient`。desktop 模式失败时不会回退 legacy。完整协议见 [Desktop Broker v2](docs/desktop-broker-protocol.md)。
...
完整连接变量和 bootstrap 入口说明见 [环境变量总览](docs/environment-variables.md)。
...
完整配置来源变量说明见 [环境变量总览](docs/environment-variables.md)。
...
Services 环境变量和 bootstrap 条件见 [环境变量总览](docs/environment-variables.md)。
...
普通异常捕获也可以在 bootstrap 阶段通过环境变量启用，完整变量表见 [环境变量总览](docs/environment-variables.md)。
...
完整日志变量、默认值和可选值见 [环境变量总览](docs/environment-variables.md)。
