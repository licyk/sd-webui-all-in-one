...
- 现有扩展补丁：`extensions.zluda`、`extensions.extension_index`、`extensions.hf_endpoint_mirror`
...
- `get_catalog()`：返回功能目录、可调设置、默认值和当前 `monkey_zoo` 注册状态。
...
`load_config_file(..., write_back=True)` 读取后会补齐 `core`、`runtime.logs` 和其他扩展默认值, 并把完整配置写回文件。补齐规则是深度合并：只有当用户值和默认值都是 object 时才递归合并；用户已有的非 object 值保持原样。
