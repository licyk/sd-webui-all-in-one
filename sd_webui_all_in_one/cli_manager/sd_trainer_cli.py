...
def export_environment(
    ...
    return output_environment_info(
        lambda: get_sd_trainer_environment_info(sd_trainer_path, include_packages),
        output,
        overwrite=overwrite,
    )
...
def register_sd_trainer(
    ...
    # check-update
    check_update_p = trainer_sub.add_parser("check-update", help="检查 SD Trainer 内核更新")
    check_update_p.add_argument("--sd-trainer-path", type=normalized_filepath, required=False, default=SD_TRAINER_ROOT_PATH, dest="sd_trainer_path", help="SD Trainer 根目录")
    ...
    # export-environment
    environment_p = trainer_sub.add_parser("export-environment", help="导出 SD Trainer 环境信息")
    environment_p.add_argument("--sd-trainer-path", type=normalized_filepath, required=False, default=SD_TRAINER_ROOT_PATH, dest="sd_trainer_path", help="SD Trainer 根目录")
