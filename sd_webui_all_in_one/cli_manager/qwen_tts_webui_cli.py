...
from sd_webui_all_in_one.base_manager.qwen_tts_webui_base.lifecycle import QwenTtsEnvCheckName
...
def update(
    ...
    _create_pre_operation_snapshot(
        qwen_tts_webui_path=qwen_tts_webui_path,
        operation_name="更新 Qwen TTS WebUI",
...
def _create_pre_operation_snapshot(
    ...
    create_pre_operation_snapshot(
        lambda: get_qwen_tts_webui_snapshot(qwen_tts_webui_path=qwen_tts_webui_path, include_packages=True),
...
def export_environment(
    ...
    return output_environment_info(
        lambda: get_qwen_tts_webui_environment_info(qwen_tts_webui_path, include_packages),
        output,
        overwrite=overwrite,
    )
...
def restore(
    ...
    restore_snapshot(
        ...
        expected_webui_type="qwen_tts_webui",
...
def launch_version_gui(
    ...
    _create_pre_operation_snapshot(
        qwen_tts_webui_path=qwen_tts_webui_path,
        operation_name="启动 Qwen TTS WebUI 版本管理 GUI",
        snapshot_enabled=snapshot_enabled,
        snapshot_dir=snapshot_dir,
...
def reinstall_pytorch(
    ...
    if not list_only:
        _create_pre_operation_snapshot(
            qwen_tts_webui_path=qwen_tts_webui_path,
            operation_name="重装 Qwen TTS WebUI PyTorch",
            snapshot_enabled=snapshot_enabled,
            snapshot_dir=snapshot_dir,
        )
...
def register_qwen_tts_webui(
    ...
    reinstall_pytorch_p.add_argument("--qwen-tts-webui-path", type=normalized_filepath, required=False, default=QWEN_TTS_WEBUI_ROOT_PATH, dest="qwen_tts_webui_path", help="Qwen TTS WebUI 根目录")
    ...
    # check-update
    check_update_p = qwen_tts_webui_sub.add_parser("check-update", help="检查 Qwen TTS WebUI 内核更新")
    check_update_p.add_argument("--qwen-tts-webui-path", type=normalized_filepath, required=False, default=QWEN_TTS_WEBUI_ROOT_PATH, dest="qwen_tts_webui_path", help="Qwen TTS WebUI 根目录")
    ...
    # export-environment
    environment_p = qwen_tts_webui_sub.add_parser("export-environment", help="导出 Qwen TTS WebUI 环境信息")
    environment_p.add_argument("--qwen-tts-webui-path", type=normalized_filepath, required=False, default=QWEN_TTS_WEBUI_ROOT_PATH, dest="qwen_tts_webui_path", help="Qwen TTS WebUI 根目录")
    ...
    # restore
    restore_p = qwen_tts_webui_sub.add_parser("restore", help="恢复 Qwen TTS WebUI 环境快照")
    add_restore_arguments(restore_p, "--qwen-tts-webui-path", "qwen_tts_webui_path", QWEN_TTS_WEBUI_ROOT_PATH)
    ...
    check_p.add_argument(
        ...
        default=None,
        dest="include_checks",
        choices=list(QwenTtsEnvCheckName),
    ...
    snapshot_gui_p = gui_sub.add_parser("snapshot-manager", help="启动 Qwen TTS WebUI 快照管理 GUI")
    add_snapshot_gui_arguments(snapshot_gui_p, "--qwen-tts-webui-path", "qwen_tts_webui_path", QWEN_TTS_WEBUI_ROOT_PATH)
