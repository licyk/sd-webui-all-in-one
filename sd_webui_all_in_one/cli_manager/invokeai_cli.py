...
from sd_webui_all_in_one.base_manager.invokeai_base.lifecycle import InvokeAIEnvCheckName
...
def export_environment(
    ...
    return output_environment_info(
        lambda: get_invokeai_environment_info(invokeai_path, include_packages),
        output,
        overwrite=overwrite,
    )
...
def register_invokeai(
    ...
    # export-environment
    environment_p = invoke_sub.add_parser("export-environment", help="导出 InvokeAI 环境信息")
    environment_p.add_argument("--invokeai-path", type=normalized_filepath, required=False, default=INVOKEAI_ROOT_PATH, dest="invokeai_path", help="InvokeAI 根目录")
    ...
    # restore
    restore_p = invoke_sub.add_parser("restore", help="恢复 InvokeAI 环境快照")
    add_restore_arguments(restore_p, "--invokeai-path", "invokeai_path", INVOKEAI_ROOT_PATH)
    ...
    check_p.add_argument(
        ...
        default=None,
        dest="include_checks",
        choices=list(InvokeAIEnvCheckName),
    ...
    snapshot_gui_p = gui_sub.add_parser("snapshot-manager", help="启动 InvokeAI 快照管理 GUI")
    add_snapshot_gui_arguments(snapshot_gui_p, "--invokeai-path", "invokeai_path", INVOKEAI_ROOT_PATH)
