...
from sd_webui_all_in_one.base_manager.sd_webui_base.lifecycle import SDWebUiEnvCheckName
...
def export_environment(
    ...
    """导出 Stable Diffusion WebUI 环境信息。
    ...
    return output_environment_info(
        lambda: get_sd_webui_environment_info(sd_webui_path, include_packages),
        output,
        overwrite=overwrite,
    )
...
def switch(
    ...
    if not list_only and (interactive_mode or branch is not None):
        _create_pre_operation_snapshot(
            sd_webui_path=sd_webui_path,
            operation_name="切换 Stable Diffusion WebUI 分支",
            snapshot_enabled=snapshot_enabled,
            snapshot_dir=snapshot_dir,
        )
    switch_sd_webui_branch(
...
def launch(
    ...
    if check_launch_env:
        try:
            ...
            traceback.print_exc()
            logger.error("检查 Stable Diffusion WebUI 运行环境时发生了错误: %s", e)
            logger.warning("该问题并非致命, 但这可能会导致 Stable Diffusion WebUI 运行时发生问题")
    ...
    try:
        ...
        logger.error("运行 Stable Diffusion WebUI 时异常退出: %s", e)
        sys.exit(1)
...
def register_sd_webui(
    ...
    # export-environment
    environment_p = sd_sub.add_parser("export-environment", help="导出 Stable Diffusion WebUI 环境信息")
    environment_p.add_argument("--sd-webui-path", type=normalized_filepath, required=False, default=SD_WEBUI_ROOT_PATH, dest="sd_webui_path", help="Stable Diffusion WebUI 根目录")
    ...
    check_p.add_argument(
        ...
        default=None,
        dest="include_checks",
        choices=list(SDWebUiEnvCheckName),
    ...
    snapshot_gui_p = gui_sub.add_parser("snapshot-manager", help="启动 Stable Diffusion WebUI 快照管理 GUI")
    add_snapshot_gui_arguments(snapshot_gui_p, "--sd-webui-path", "sd_webui_path", SD_WEBUI_ROOT_PATH)
    ...
    add_auto_mirror_argument(ext_install_p)
    ext_install_p.set_defaults(
        func=with_auto_mirror(
            lambda args: install_extension(
                sd_webui_path=args.sd_webui_path,
