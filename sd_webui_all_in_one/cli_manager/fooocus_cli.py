...
from sd_webui_all_in_one.base_manager.fooocus_base.lifecycle import FooocusEnvCheckName
...
def _create_pre_operation_snapshot(
    ...
    create_pre_operation_snapshot(
        lambda: get_fooocus_snapshot(fooocus_path=fooocus_path, include_packages=True),
...
def launch(
    ...
    if check_launch_env:
        try:
            ...
            traceback.print_exc()
            logger.error("检查 Fooocus 运行环境时发生了错误: %s", e)
            logger.warning("该问题并非致命, 但这可能会导致 Fooocus 运行时发生问题")
    ...
    try:
        ...
        logger.error("运行 Fooocus 时异常退出: %s", e)
        sys.exit(1)
...
def launch_version_gui(
    ...
    _create_pre_operation_snapshot(
        fooocus_path=fooocus_path,
        operation_name="启动 Fooocus 版本管理 GUI",
        snapshot_enabled=snapshot_enabled,
        snapshot_dir=snapshot_dir,
...
def reinstall_pytorch(
    ...
    if not list_only:
        _create_pre_operation_snapshot(
            fooocus_path=fooocus_path,
            operation_name="重装 Fooocus PyTorch",
            snapshot_enabled=snapshot_enabled,
            snapshot_dir=snapshot_dir,
        )
...
def register_fooocus(
    ...
    reinstall_pytorch_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    install_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    update_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    # check-update
    check_update_p = fooocus_sub.add_parser("check-update", help="检查 Fooocus 内核更新")
    check_update_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    check_p.add_argument(
        ...
        default=None,
        dest="include_checks",
        choices=list(FooocusEnvCheckName),
    ...
    launch_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    snapshot_gui_p = gui_sub.add_parser("snapshot-manager", help="启动 Fooocus 快照管理 GUI")
    add_snapshot_gui_arguments(snapshot_gui_p, "--fooocus-path", "fooocus_path", FOOOCUS_ROOT_PATH)
    ...
    model_lib_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    model_url_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    model_list_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
    ...
    model_uninstall_p.add_argument("--fooocus-path", type=normalized_filepath, required=False, default=FOOOCUS_ROOT_PATH, dest="fooocus_path", help="Fooocus 根目录")
