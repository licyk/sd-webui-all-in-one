...
def test_main_returns_zero_and_uses_requested_cwd(monkeypatch):
...
def test_main_returns_zero_after_forced_termination(monkeypatch):
    process = FakeProcess(
        [
...
def test_cli_returns_zero_when_launch_fails():
    result = subprocess.run(
        [
            ...
            "1",
            "--",
            "sd-webui-all-in-one-command-that-does-not-exist",
        ],
        check=False,
    )
...
def test_main_returns_zero_for_invalid_invocation(argv):
